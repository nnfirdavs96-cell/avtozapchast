...
## 16. Маркетплейс (мультипродавец) — Фазы 1–3a + buy-box
...
### 16.7 Buy-box — конкуренция продавцов и варианты товара ✅

Главное ограничение маркетплейса до этого этапа: `parts.part_number` был **уникален по
всей базе**. Выложил товар один продавец — второй получал отказ. Конкуренции не
существовало, то есть маркетплейса, по сути, тоже.

**Модель — три уровня** (`includes/parts/grouping.php`):

```
семейство         варианты одного продавца      product_group_id
  └── карточка    тот же товар у конкурентов    product_id        ← buy-box тут
        └── предложение продавца                parts.id          ← это кладут в корзину
```

Строка `parts` — это **предложение**: своя цена, наличие, фото. `parts.id` остаётся
идентификатором предложения, поэтому все 8 внешних ключей (корзина, позиции заказов,
избранное, отзывы, совместимость, переводы) продолжают работать без миграции. В корзину
и должно класться предложение конкретного продавца по конкретной цене.

**Колонки** (`sql/migrate_buybox.php`, идемпотентна):

| Колонка | Что |
|---|---|
| `part_key` | артикул в каноническом виде — «90915-YZZE1» и «90915 yzze1» опознаются как один товар |
| `product_id` | номер карточки; **NULL = товар пока один в своём роде** |
| `product_group_id` | номер семейства (варианты) |
| `is_unique_item` | б/у, разборка, позиция без артикула — не сводится с чужими |
| `part_attributes` | гибкие атрибуты: `axis` (ось варианта) / `spec` (характеристика) |

**Ключи через COALESCE.** Карточка — `COALESCE(product_id, id)`, семейство —
`COALESCE(product_group_id, id)`. Одиночный товар автоматически «группа из самого себя»,
и нигде не нужен особый случай «а если NULL».

⚠️ Ключ семейства **не** падает через `product_id`, хотя соблазн есть. Карточка и
семейство — разные оси: у главного варианта `product_group_id` пуст, а `product_id`
вполне может быть заполнен (тот же артикул продаёт кто-то ещё). Тогда его ключ семейства
стал бы номером чужой карточки, и семейство развалилось бы надвое. Эта ошибка была
допущена на этапе 1 и исправлена на этапе 5.

⚠️ **Внешнего ключа на `product_id` намеренно нет.** Это номер группы, которым оказался
id первого выложившего, а не ссылка на строку. Если тот продавец удалит свой товар,
остальные предложения обязаны остаться сгруппированными — внешний ключ их бы раскидал
(`SET NULL`) или удалил (`CASCADE`).

**Правило buy-box** (`partsBuyBoxOrder`): сначала «в наличии», среди них — дешевле.
Намеренно совпадает с правилом для предложений AutoEuro (`offersByOem`): покупатель не
должен видеть на одной витрине две разные логики выбора.

**Отбор витрины** (`partsBuyBoxSource`) — снизу вверх, три уровня оконных функций:
предложение → одно на продавца → победитель карточки → (опционально) победитель
семейства. Возвращает подзапрос под псевдонимом `p`, которым заменяется `FROM parts p`,
поэтому условия и сортировки вызывающего кода не менялись.

- `$groupFamily = true` — просмотр каталога, категории, главная;
- `false` — поиск и подсказки: человек ищет «бежевый чехол», и схлопывание семейства
  подменило бы его чёрным.

**Требуются оконные функции** (MySQL 8+ / MariaDB 10.2+). Наличие проверяется пробным
запросом по настоящей колонке настоящей таблицы, а не разбором `VERSION()`. Если их
нет — витрина работает плоским списком, как до buy-box.

| Файл | Что делает |
|---|---|
| `includes/parts/grouping.php` | вся логика: ключи, buy-box, предложения карточки, семейства, атрибуты, дубли |
| `sql/migrate_buybox.php` | миграция + `--check` (диагностика) + `--report` (что склеилось) + `--regroup` |
| `sql/seed_test_sellers.php` | тестовые магазины для проверки конкуренции; удаление только своего |
| `catalog/part.php` | блок «Предложения продавцов», переключатель вариантов, характеристики, canonical на победителя |

**Грабли, наступленные и закрытые:**
- пробник оконных функций делал `OVER (ORDER BY 1)` — SQLite это выполняет, MySQL падает;
  вся витрина тихо уходила на запасной путь;
- `<=>` и `ON DUPLICATE KEY` — MySQL-специфика, из-за которой функции приходилось
  проверять «зеркалами» в тестах, а зеркало проверяет копию, а не код;
- PDO по умолчанию шлёт число строкой; MySQL приводит тип молча, другие СУБД нет;
- галочка «уникальный товар» изначально была только у продавца — правка товара в админке
  молча сбрасывала бы флаг;
- три страницы (`catalog/category.php`, `api/search.php`, блок «похожие товары») **не
  проверяли модерацию** — неодобренные листинги продавцов были видны покупателю.

⚠️ **Известное ограничение.** Семейство принадлежит продавцу, а карточку может выиграть
конкурент. Тогда линейка разрывается на несколько плиток, и их число зависит от того, кто
выиграл buy-box. Ничего не теряется (вариант доступен в блоке предложений), но лечится
только переносом вариантов на общую карточку — полная модель Ozon, отдельная задача.
