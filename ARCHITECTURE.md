...
`site_settings` (по умолчанию `0`). Выплаты не реализованы (Фаза 3) — поле заложено заранее.
...
- **Маркетплейс — Фазы 1 и 2 готовы.** Продавцы, их товары, модерация, витрина магазина,
  **заказы с разбивкой по продавцам** (подзаказы, статусы, комиссия). Не сделано:
  онлайн-оплата и **выплаты** продавцам (Фаза 3, упирается в эквайринг), отзывы о
  продавцах, возвраты/споры, **buy-box** (одна деталь — предложения нескольких продавцов;
  мешает `parts.part_number UNIQUE` по всей базе).
...
## 16. Маркетплейс (мультипродавец) — Фазы 1–2

Цель — площадка, где **сторонние продавцы выкладывают свои товары** (модель Ozon/WB),
в нише автозапчастей. **Фазы 1 и 2 завершены**: продавец регистрируется, выкладывает
товар, проходит модерацию, получает заказ и ведёт его статус. Деньги — Фаза 3.
