...
| Buy-box | `includes/parts/grouping.php`, `sql/migrate_buybox.php` | один артикул у разных продавцов → одна карточка; победитель «в наличии → дешевле»; варианты (цвет/размер/возраст) |
...
- **Артикул больше НЕ уникален по всей базе.** У разных продавцов он и должен совпадать —
  это и есть конкуренция. Нельзя выложить свой же артикул дважды. Сравнение идёт по
  `part_key` (только буквы и цифры, верхний регистр), а не по строке.
- **Группировка требует оконных функций** (MySQL 8+). Без них витрина работает плоским
  списком — молча, поэтому есть `php sql/migrate_buybox.php --check`.
...
- **Buy-box** (конкуренция продавцов, варианты товара) — **готов**, этапы 1–7.
- **Фаза 4** (отзывы о продавцах, возвраты/споры) — не начата.
