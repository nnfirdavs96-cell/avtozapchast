...
| `includes/parts/` | Блоки витрины: `supplier_cards.php` (карточки AutoEuro + модалка + «Показать ещё»), `supplier_card_render.php` (общий рендер карточки), **`grouping.php`** (buy-box: ключи карточки и семейства, выбор победителя, предложения, варианты, атрибуты) |
...
### Маркетплейс, buy-box — конкуренция и варианты (`sql/migrate_buybox.php`)
| Таблица / колонка | Что |
|---|---|
| `parts.part_key` | артикул в каноническом виде (только буквы и цифры, верхний регистр) — «90915-YZZE1» и «90915 yzze1» это один товар |
| `parts.product_id` | номер КАРТОЧКИ: тот же товар у других продавцов. **NULL = товар пока один в своём роде** |
| `parts.product_group_id` | номер СЕМЕЙСТВА: тот же товар в другом исполнении (цвет, объём, возраст) |
| `parts.is_unique_item` | б/у, разборка, позиция без артикула — не сводится с чужими |
| `part_attributes` | гибкие атрибуты: `kind` = `axis` (ось варианта, своя цена и наличие) / `spec` (характеристика); UNIQUE(part_id, kind, name) |
| **снято** | глобальная уникальность `parts.part_number` — из-за неё второй продавец не мог выложить занятый артикул |

> Ключи считаются на лету: карточка — `COALESCE(product_id, id)`, семейство —
> `COALESCE(product_group_id, id)`. Одиночный товар автоматически «группа из самого себя».
> **Внешнего ключа на `product_id` нет намеренно** — это номер группы, а не ссылка на
> строку: если первый выложивший удалит товар, остальные должны остаться сгруппированными.
> Требуются оконные функции (MySQL 8+ / MariaDB 10.2+); без них витрина работает плоским
> списком, как до buy-box.
...
- **Buy-box готов** (этапы 1–7): один артикул у разных продавцов, витрина сводит их в одну
  карточку и выбирает лучшее предложение, на карточке блок «продают N продавцов», варианты
  (цвет/размер/возраст) с переключателем. ⚠️ Остаётся ограничение: семейство принадлежит
  продавцу, поэтому если карточку выиграл конкурент, линейка разрывается на несколько
  плиток. Лечится переносом вариантов на общую карточку — полная модель Ozon.
...
### #368–#380 · Фаза 3a (балансы и выплаты) и buy-box: конкуренция продавцов и варианты (22–24 августа)

- **#380** (2026-08-24) — feat(marketplace): buy-box этап 6 — каталог группирует по семейству; в поиске группировка намеренно выключена (иначе «бежевый чехол» подменился бы чёрным); задокументировано ограничение: если карточку выиграл конкурент, линейка продавца разрывается на несколько плиток
- **#379** (2026-08-24) — feat(marketplace): buy-box этап 5 — варианты товара (семейство задаётся ссылкой на свой товар, гибкие атрибуты `axis`/`spec`); **исправлена ошибка этапа 1** — ключ семейства падал через `product_id` и разваливал семейство надвое; убран MySQL-специфичный `<=>` и `ON DUPLICATE KEY`, из-за которых тесты проверяли «зеркала», а не код
- **#378** (2026-08-24) — feat(marketplace): buy-box этап 7 — тестовые продавцы; магазину без наличия даётся самая низкая цена, чтобы проверка доказывала правило buy-box; удаление трогает только созданное скриптом
- **#377** (2026-08-24) — feat(marketplace): buy-box этап 4 — формы товаров: снят запрет «артикул уникален по всей базе», теперь нельзя выложить свой же артикул дважды; галочка «уникальный товар» во всех трёх формах (иначе правка в админке сбрасывала флаг)
- **#376** (2026-08-24) — feat(marketplace): buy-box этап 3 — блок «Предложения продавцов» на карточке, canonical на победителя; исправлен третий пропуск модерации (блок «похожие товары»)
- **#375** (2026-08-24) — fix(marketplace): пробник оконных функций делал `OVER (ORDER BY 1)` — SQLite выполняет, MySQL падает, и вся витрина тихо уходила на запасной путь без группировки; добавлен режим `--check`
- **#374** (2026-08-24) — feat(marketplace): buy-box этап 2 — витрина группирует предложения, одно предложение на продавца, счётчики по карточкам; исправлены два пропуска модерации (`catalog/category.php`, `api/search.php`); поиск фото поставщика переведён на индексированный `part_key`
- **#373** (2026-08-24) — feat(marketplace): режим `--report` — увидеть, что склеилось в одну карточку, до показа на витрине
- **#372** (2026-08-24) — feat(marketplace): buy-box этап 1 — модель товара: снята глобальная уникальность артикула, добавлены `part_key`/`product_id`/`product_group_id`/`is_unique_item` и `part_attributes`
- **#370–#371** (2026-08-23) — feat(sql): накат миграций без пароля в командной строке (`~/.bash_history` на бою append-only); fix(admin): модалка выплаты разворачивалась в поток страницы — в админке `custom.css` подключался без cache-busting
- **#369** (2026-08-23) — feat(marketplace): **Фаза 3a** — журнал `seller_ledger`, балансы продавцов, реестр `seller_payouts`, выгрузка CSV; эквайринг для этого не нужен
- **#368** (2026-08-23) — docs: актуализация всех шести документов до PR #367
...
- **#316** (2026-08-07) — style(catalog): сайдбар фильтров как карточка с тенью + `position: sticky` на десктопе; раздельный поиск обратно (владелец отменил объединение); плавный zoom фото при наведении вместо иконки «глаз»
- **#315** (2026-08-06) — feat(header,catalog): единая полоса поиска (позже отменена в #316), фикс зазора карточек, убрана иконка быстрого просмотра
- **#314** (2026-08-06) — fix(header,catalog): десктоп-высота VIN-поля 50px (было 0, схлопывалось), кнопка каталога без пустоты
- **#313** (2026-08-06) — style(catalog): фото фиксированный квадрат `aspect-ratio: 1/1` (позже 4:3), единый ритм отступов
- **#312** (2026-08-06) — style(header): поле VIN на десктопе → 300px, плейсхолдер «VIN — 17 символов» не обрезается
- **#311** (2026-08-06) — style(catalog): кнопка всегда в потоке карточки (тема прячет `position: absolute; bottom: -55px` — недоступно на тач-экранах); Rubik-типографика; сердечко-квадрат
- **#310** (2026-08-06) — style(catalog): постоянная тень у карточек-плиток (не только по наведению)
- **#309** (2026-08-06) — style(catalog): карточки как плитки — рамка `#ececf0`, скругление 12px, отступы 22/16/14px десктоп/планшет/телефон
- **#308** (2026-08-06) — fix(catalog): дублирование `.grid_content`/`.list_content` (моё правило перебивало скрытие темы одинаковой специфичностью); убран login-wall у «В корзину» — гость получает нормальную кнопку
- **#307** (2026-08-06) — fix(home): промо-карточка «Товары со скидкой» — свой класс `.promo_deal` вместо inline-стилей, конфликтовавших с общим правилом карточек
- **#306** (2026-08-05) — fix(mobile): поле VIN раздувалось в высокий блок (базовое `flex: 0 1 210px` работало как высота в колонке); шапка не переливается за край
- **#305** (2026-08-05) — feat(mobile): сегментированный переключатель «Запчасти / По VIN» — одно поле за раз вместо двух подряд
- **#304** (2026-08-05) — style(mobile): убрать «белый хвост» под полем поиска (тема ставит кнопку `position: absolute` с фикс. высотой 40px, форма выше)
- **#303** (2026-08-05) — fix(mobile): логотип и иконки в одной строке (`display: contents` на обёртках), равная высота блока цены
- **#302** (2026-08-05) — style: `.single_product` — колонка на всю высоту, кнопки через `margin-top: auto`; логотип не в углу
- **#301** (2026-08-05) — style(products): выровнять кнопки карточек главной + вкладки «Бестселлеры/…» в один ряд с прокруткой на мобильном
- **#300** (2026-08-05) — style(header): единый стиль иконок мобильной шапки (призрачные квадраты 42×42), логотип 52→34px
- **#299** (2026-08-05) — fix(header): сломанная форма VIN на мобильном (тема ставит `flex-direction: column-reverse` — кнопка уезжала над полем)
- **#298** (2026-08-05) — fix(assets): UTF-8 для `.css`/`.js` (`@charset` + `AddCharset` в `.htaccess`) — русские комментарии в `custom.css` больше не кракозябры
- **#297** (2026-08-04) — fix(header): читаемый выпадающий список языков (`.dropdown_language` — тёмный `#222` вместо белого, красный акцент на hover)
- **#296** (2026-08-04) — style(home): гарантированно убрать пустоту под категориями (`!important` + обнуление внутренних обёрток)
- **#295** (2026-08-04) — style(home): ужать разрыв категории → товары ещё сильнее (10px / 26px)
- **#294** (2026-08-04) — style(home): убрать ~160px пустоты между категориями и товарами (тема давала `mb-80` + `padding-top: 75px`)
- **#293** (2026-08-04) — fix(i18n): «Наш склад в Худжанде» (ru/tg) — в английском уже правильно; логика AutoEuro/Москва намеренно не тронута
- **#292** (2026-08-04) — fix(header/vin): резиновая ширина поиска (было фикс. 860px — поле «Артикул» схлопывалось); модалка выбора доставки без стилей на страницах без поиска (CSS был в условном блоке); адаптив планшет/телефон
- **#291** (2026-08-04) — docs: полная сверка `.md` с кодом; блок «Какой документ для чего» в README; CATALOG_PLAN исправлен (не «к разработке не приступали», а «реализовано»); CHANGES помечен как исторический
...
| [`sql/migrate_buybox.php`](sql/migrate_buybox.php) | Buy-box: снимает уникальность артикула, добавляет ключи группировки и атрибуты. Режимы `--check` (работает ли группировка на этом сервере), `--report` (что склеилось в одну карточку), `--regroup` (пересчёт). | parts, part_attributes |
| [`sql/seed_test_sellers.php`](sql/seed_test_sellers.php) | Тестовые магазины с товарами на тех же артикулах — чтобы увидеть конкуренцию продавцов вживую. Удаляет только созданное собой (`--remove --yes`). | sellers, users, parts |
