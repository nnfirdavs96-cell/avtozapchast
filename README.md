...
| #263 | **Прогрев библиотеки каталога — CLI-скрипт `superadmin/catalog_library_seed.php`.** В отличие от кнопки/cron (только добирают схемы у уже известных авто), скрипт сам открывает новые марки/модели/авто через `pcBrands()/pcModels()/pcCars()` (без VIN, «по параметрам») и сохраняет их в `catalog_library_*`. Список марок под рынок Таджикистана задаётся в начале файла; параметры запуска — `budget_cars/schemes_per_car/pause_ms`. ⚠️ Каждое новое авто — платный запрос к Parts-Catalogs (тариф за авто/сутки); запускать осознанно (не по расписанию), удобно на тестовом ключе без лимита |
...
- **Реальные цены каталога** — сейчас цена подставляется только для артикулов, совпавших со своим складом (остальное — «под заказ»); для цен на весь каталог нужен прайс поставщика, сопоставляемый по артикулу.
