...
запчастей и расширенного VIN-декодера уже встроен. Клиент покупает подписку
(рекомендуется **PartsAPI.ru** — даёт TecDoc-каталог + VIN-декодер одним
ключом, работает в СНГ/России), вставляет ключ в админке — и каталог
оживает. **Пока ключ не введён и тумблер выключен — сайт работает как
обычно**: бесплатный декодер NHTSA + локальная база WMI + собственный каталог
товаров. Ничего не ломается.
...
| Блок | Что задаётся |
|------|--------------|
| **Настройки VIN API** | Провайдер (`NHTSA` / `PartsAPI` / другой), URL-шаблон, ключ, таймаут |
| **Каталог запчастей (внешний API)** | Тумблер вкл/выкл, провайдер, URL-шаблон, ключ, таймаут |
| **Проверить соединение** | Тестовый запрос к каталогу + фрагмент ответа сервера |

### URL-шаблоны и плейсхолдеры

URL запроса задаётся **шаблоном** с подстановками — это позволяет подключить
любого провайдера без правки кода:

- `{VIN}` — подставляется проверяемый VIN (URL-кодируется)
- `{KEY}` — подставляется API-ключ (URL-кодируется)

Ключ дополнительно отправляется в заголовках `Authorization: Bearer …` и
`X-Api-Key: …` — это покрывает и query-param, и header-авторизацию.

Примеры (уточняйте точный формат в документации провайдера):
```
VIN-декодер:  https://api.partsapi.ru/?method=VINdecode&key={KEY}&vin={VIN}&format=json
Каталог:      https://api.partsapi.ru/?method=PartsByVIN&key={KEY}&vin={VIN}&format=json
```
...
| `vin_api_url` | URL-шаблон VIN-декодера | — |
| `vin_api_key` | Ключ VIN-декодера | — |
| `vin_api_timeout` | Таймаут, сек | `8` |
| `catalog_api_enabled` | Тумблер каталога `0`/`1` | `0` |
| `catalog_api_provider` | `partsapi` · `custom` | `partsapi` |
| `catalog_api_url` | URL-шаблон каталога | — |
| `catalog_api_key` | Ключ каталога | — |
| `catalog_api_timeout` | Таймаут, сек | `10` |

> Миграции БД не требуются — всё хранится в существующей таблице `site_settings`.

### Как это устроено в коде

- `includes/catalog_api.php` — класс `CatalogApi`: запрос к каталогу по VIN,
  **защитный разбор** ответа (распознаёт распространённые имена полей
  PartsAPI / TecDoc / generic-JSON: `name/articleNumber/brand/price/…`),
  тест соединения. Все методы инертны, пока `catalog_api_enabled=0`.
- `includes/vin_service.php` — провайдер `partsapi`/`custom` ходит по
  настраиваемому URL-шаблону (`{VIN}`/`{KEY}`), результат сливается с
  локальным декодом.
- `pages/vin.php` — если каталог включён, под «своими» товарами выводится
  блок **«Запчасти из каталога TecDoc»** с данными провайдера.

### Передача клиенту

1. Клиент регистрируется у провайдера (PartsAPI.ru) и покупает тариф.
2. В **Суперадмин → VIN-поиск → Настройки API** вставляет URL-шаблон + ключ.
3. Жмёт **«Проверить соединение»** — убеждается, что приходит JSON.
4. Включает тумблер **«Включить каталог по API»** и сохраняет.

> Если ответ провайдера приходит, но позиции не распознаются — точные имена
> полей видны в блоке «Ответ сервера (фрагмент)»; их сопоставление
> донастраивается в `CatalogApi::normalizeParts()`.
...
- [x] **VIN-поиск + каталог по API** — интеграционный слой готов (см. раздел «Интеграция внешнего каталога»). Осталось купить подписку (PartsAPI / TecDoc), вставить ключ и URL в **Суперадмин → VIN-поиск → Настройки API** и включить тумблер.
