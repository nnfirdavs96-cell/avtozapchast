...
**Актуальный статус (август 2026):** каталог по VIN работает боевым (Parts-Catalogs /
Tradesoft: VIN → авто → узлы → схемы → детали + поиск «По параметрам»); цены — свой склад →
AutoEuro (в сомони, с наценкой и доставкой Москва→Худжанд); заказ детали поставщика —
заявка менеджеру. Не подключено осознанно: автозаказ у AutoEuro (`create_order`) и
онлайн-эквайринг — см. «Что осталось».
...
| #288–#290 | **Диагностика VIN, фикс каскада «По параметрам», поиск в шапке.** (#288) Тест соединения Parts-Catalogs при нераспознанном VIN показывает сырой ответ именно от `v1/car/info/` — видно причину (нет в базе / тариф), а не только «не распознан». (#289) **Фикс:** каскад «По параметрам» показывал короткий заглушечный список вместо 74 брендов — пустой список брендов однажды закэшировался и «залипал» на 30 дней; теперь пустой кэш = промах (перезапрос вживую) и пустое не кэшируется — самовосстановление без ручной чистки. (#290) Поиск больше не уезжает при прокрутке: в «прилипающую» полосу добавлен компактный поиск; и вверху, и в прилипшей полосе — **два поля: запчасти + отдельный VIN** |
