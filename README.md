...
- **Реальный подбор «По параметрам» (TecDoc).** Сейчас каскад Марка→Модель→Год работает на курируемом списке и ведёт в поиск/к эксперту. После покупки платного ключа PartsAPI можно включить настоящий vehicle-selector TecDoc (новые методы в `CatalogApi` + эндпоинт каскада) — задел и место под это оставлены.
