...
1. Карточка авто отрисована — JS асинхронно вызывает `api/vin_catalog.php?vin=…`
   (перебор групп долгий, поэтому не блокирует страницу — крутится прелоадер).
2. `CatalogApi::searchByVin()`:
   - проверяет кэш `partsapi_catalog_cache`;
   - перебирает товарные группы (ходовые первыми), для каждой —
     `getPartsbyVIN(vin, type, cat)`;
   - разбирает `parts` на пары бренд/артикул, дедуплицирует;
   - сопоставляет артикулы со своим складом → обогащает ценой/наличием/ссылкой;
   - кэширует и возвращает.
3. JS группирует позиции по узлам (Кузов, Тормоза, Освещение…) и рисует таблицу:
   свои товары — с ценой и «в корзину», прочие — «под заказ» + ссылка «найти».
...
- `includes/catalog_api.php` — класс `CatalogApi`: перебор групп
  ...
  кэш (версионируемый). Инертен, пока `catalog_api_enabled=0`.
...
- `pages/vin.php` — карточка авто рендерится сразу, каталог подгружается
  **асинхронно**, группируется по узлам.
...
| оба http=200, в теле «limit» | исчерпан суточный лимит ключа | платный тариф / ждать сброса |
