...
> 🗺 **Карта кода — [`ARCHITECTURE.md`](ARCHITECTURE.md).** Чтобы не искать файлы вслепую:
> технический стек, структура папок, система «VIN + каталог» (провайдеры/эндпоинты/фронт),
> корзина, настройки `site_settings`, таблицы БД и индекс **«хочу изменить X → файл Y»**.
> План каталога — [`CATALOG_PLAN.md`](CATALOG_PLAN.md).
