...
> **Стадия:** все PR #219–#229 **влиты в `main`**; развёртывание на боевом сервере — `git pull` в `~/public_html`. Правки #219–#229 не трогают секреты (ключи API/пароли хранятся только в БД `site_settings`, в репозиторий не попадают).
>
> **В работе:** режим **«По параметрам»** использует тот же каталог/схемы/вырезки, что и VIN (подтверждено). Для отдельных авто без данных у Parts-Catalogs (`oemNodesForCar` = 0 узлов, напр. спецтехника) планируется понятное сообщение «оригинальный каталог для этого авто недоступен» вместо пустого блока.
