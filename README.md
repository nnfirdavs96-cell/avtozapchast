...
### 📌 Что осталось (по состоянию на 06.2026)
...
- **Parts-Catalogs (OEM-схемы) — подготовлено (PR #204), ждёт платного ключа.** Провайдер, адаптер, эндпоинты, панель кликабельных взрыв-схем и настройки готовы. После покупки: Суперадмин → VIN → провайдер «Parts-Catalogs» → ключ → «Проверить». На боевом ключе останется подтвердить 2–3 мелочи (формат авторизации сырой/`Bearer`, точные имена полей схемы, кросс-доменную отдачу картинок) — вынесены в один метод, 1-строчная правка, сайт при расхождении не ломается (деградация до «картинка + список»).
- Реальный подбор «По параметрам» (Марка→Модель) на данных провайдера — у Parts-Catalogs есть `catalogs/models/cars2`; можно завести каскад из реальных каталогов вместо курируемого списка (следующий шаг после подтверждения ключа).
- Реальный платёжный шлюз: сейчас «Оплата онлайн» фиксирует способ и применяет скидку, а приём платежа подключается отдельной интеграцией.
- Полная локализация ru/tg/en страницы VIN: новый «каркас» переведён через `t()`, но часть текста в блоке результатов (карточка авто, оригинальный каталог, схемы) пока на русском.
- Гостевая корзина (покупка без обязательного входа) — требует смены модели корзины (`api/cart.php` сейчас жёстко привязан к `user_id`).
- Реальный платёжный шлюз: сейчас «Оплата онлайн» фиксирует способ и применяет скидку, а приём платежа подключается отдельной интеграцией.
- Полная локализация ru/tg/en страницы VIN: новый «каркас» переведён через `t()`, но часть текста в блоке результатов (карточка авто, оригинальный каталог) пока на русском.
