...
**Что подтверждается на боевом ключе** (изолировано, 1-строчная правка, деградация без поломок): формат заголовка авторизации (сырой ключ vs `Bearer` — метод `authHdr()`), точные имена полей схемы (парсер `fetchScheme()`), кросс-доменная отдача картинок (при блокировке — добавить мини-прокси).
