...
### 📚 Какой документ для чего (чтобы не искать вслепую)

| Документ | Когда открывать | Что внутри |
|---|---|---|
| **README.md** (этот) | «Как установить / настроить / что уже умеет сайт» | установка, настройка, роли, инструкции владельцу, **журнал изменений** (все PR), **«Что осталось»** |
| [**ARCHITECTURE.md**](ARCHITECTURE.md) | «В каком файле лежит X» | карта кода: стек, все файлы и функции, настройки `site_settings`, таблицы БД, контракты API, индекс **«хочу изменить X → файл Y»** |
| [**NAVIGATION.md**](NAVIGATION.md) | «Куда кликать / что за файл» | навигация по сайту и админке, карта файлов с описанием, **хронология PR** |
| [**CATALOG_PLAN.md**](CATALOG_PLAN.md) | «Как устроен каталог по VIN» | архитектура «каталог + цены» (мост = OEM-номер), провайдеры, **статус этапов** |
| [**CLAUDE.md**](CLAUDE.md) | «Правила работы в этом репозитории» | краткий контекст проекта, жёсткие правила (секреты, ветки, запрет парсинга), ключевые подсистемы, **текущая стадия** |
| [`CHANGES.md`](CHANGES.md) | — | ⚠️ исторический документ одной сессии (июль 2026), **не дополняется**; в начале — сводка текущей стадии проекта |

**Актуальный статус (август 2026, PR #380):**
- **Каталог по VIN** работает боевым (Parts-Catalogs / Tradesoft: VIN → авто → узлы → схемы → детали + поиск «По параметрам» с полным каскадом 74 брендов).
- **Библиотека каталога** — 96 машин / 87 деревьев узлов / 10 099 схем; повторные заходы обслуживаются **без запросов к API**.
  ⚠️ **Сбор новых машин остановлен:** тариф израсходован (999/1000), правила расхода запросов не подтверждены поставщиком. См. «Обслуживание библиотеки каталога».
- **Поиск у поставщика AutoEuro** — по **артикулу** живьём, по **названию/бренду** через свой словарь `ae_part_dictionary` (229 127 позиций из официального прайса), кнопка «Показать ещё» 8→16→24. Цена всегда живая.
- **Цены** — свой склад → AutoEuro (в сомони, с наценкой, надбавкой и днями доставки Москва→Худжанд).
- **Заказ детали поставщика** — модалка выбора доставки → заявка менеджеру в переписку (`api/vin_order_request.php`).
- **Маркетплейс** — Фаза 1 (продавцы, товары, модерация), Фаза 2 (заказы с разбивкой по продавцам, статусы, комиссия) и Фаза 3a (балансы продавцов и реестр выплат) **готовы**; сквозная проверка на боевом отложена. Онлайн-оплата (Фаза 3b) ждёт эквайринга.
- **Buy-box готов** — один артикул у разных продавцов сводится в одну карточку, витрина выбирает лучшее предложение («в наличии → дешевле»), на карточке блок «продают N продавцов», варианты товара (цвет/размер/возраст) с переключателем. Есть скрипт тестовых продавцов для проверки.
- **UI/UX прошёл 30+ итераций мобильной адаптации и полировки:** двойной поиск (с переключателем на мобильном), плитки-карточки каталога с тенями и sticky-сайдбаром, полностью адаптивная модалка, чат, чек-автосообщения, авто-формат телефона, тексты о магазине переведены на Худжанд.
- **Не подключено осознанно** (нужно решение владельца): автозаказ у AutoEuro (`create_order`) — денежная ветка; онлайн-эквайринг картой/QR — нужен договор с банком Таджикистана. См. «Что осталось».
...
20. [🔎 Поиск по поставщику AutoEuro (артикул + название)](#-поиск-по-поставщику-autoeuro-по-артикулу-и-по-названию)
21. [🗂 Обслуживание библиотеки каталога (Parts-Catalogs)](#-обслуживание-библиотеки-каталога-parts-catalogs)
22. [🏪 Маркетплейс (мультипродавец) — Фазы 1–3a](#-маркетплейс-мультипродавец--фазы-13a)
...
# МАРКЕТПЛЕЙС (Фаза 1): роль seller, таблица sellers, parts.seller_id/moderation_status
# Только добавления — существующие товары остаются видимыми (moderation_status DEFAULT 'active')
mysql -u avtouser -p avtozapchast < sql/marketplace_phase1.sql
```

> Если доступа к консоли `mysql` нет, ту же миграцию можно накатить через сам сайт
> (креды берутся из `config/`):
> ```bash
> php -r 'require "config/config.php"; $db=getDB();
>   foreach (explode(";", file_get_contents("sql/marketplace_phase1.sql")) as $q) {
>     $q = trim(preg_replace("/^\s*--.*$/m", "", $q)); if ($q==="") continue;
>     try { $db->exec($q); echo "OK\n"; } catch (Exception $e) { echo "SKIP: ".$e->getMessage()."\n"; } }'
> ```
...
В системе **5 ролей**. Каждая видит свой набор страниц:

> **Продавец (`seller`)** — роль маркетплейса: свой магазин и свои товары в кабинете `/seller/`.
> Не входит в таблицу ниже, т.к. не относится к персоналу: см. раздел «Маркетплейс».
...
| **Продавец** | **Кабинет продавца** | **`/seller/index.php`** |
...
**Последние (август 2026) — поиск по поставщику, каталог и маркетплейс:**

| PR | Что сделано |
|----|-------------|
| #367 | **Маркетплейс, Фаза 2**: заказ бьётся на подзаказы по продавцам (`order_sellers`), кабинет заказов продавца со статусами, разбивка у покупателя, комиссия/выплата у админа. Комиссия фиксируется в момент заказа |
| #366 | Покупателю больше не видны поставщик и наша квота — вместо технической ошибки нейтральный текст «напишите менеджеру» |
| #365 | Починка машин с **протухшим токеном комплектации** (11 авто падали с `404 «Комплектация не найдена»`) — единственное лечение — свежее декодирование VIN |
| #359, #363, #364 | Карточки авто в суперадминке: читаемая вёрстка (бейдж в потоке, заголовок в 2 строки, приглушённый водяной знак), действия «Пересчитать» / «Собрать схемы» прямо с плитки, клик по названию открывает карточку; не рисуем «Схемы: 1 из 0» |
| #357, #358, #360–#362 | Надёжность пакетного пересбора: `dbKeepAlive()` против `MySQL server has gone away`, обход **до** замены данных (перестали теряться деревья), LEFT JOIN (машина без дерева не выпадает из очереди навсегда), «дерево ровно на лимите ≠ обрезанное», `flock` против параллельных прогонов |
| #356 | `catalog_library_rebuild.php` — пакетный пересбор библиотеки (`plan`/`recount`/`schemes`/`revin`/`all`), возобновляемый |
| #351–#355 | Замер реального размера дерева (`pc_tree_probe`, Lexus: **229** узлов вместо 120), проба квоты, лимит/глубина вынесены в настройки (`catalog_nodes_limit`=300, `catalog_nodes_depth`=5), карточки авто в библиотеке; фикс фатала от лениво загружаемого `PartsCatalogsAdapter` |
| #350 | Кнопка «Показать ещё» в блоке поставщика (8 → +8), цена подтягивается живьём из AutoEuro |
| #345–#349 | **Маркетплейс, Фаза 1**: роль `seller` + таблица `sellers` + `parts.seller_id/moderation_status`; регистрация «Покупатель/Продавец»; модерация продавцов; кабинет продавца с добавлением товаров; модерация листингов; «Продавец: Магазин» на карточке + страница магазина |
| #343–#344 | Импорт прайс-листа AutoEuro (CSV) в словарь названий + `FULLTEXT`-поиск; кнопка «Показать ещё» (8 → 16 → 24…) |
| #339–#342 | Поиск по **артикулу** добирает предложения AutoEuro карточками с фото; поиск по **названию** через словарь `ae_part_dictionary`; CLI-добор названий; фиксы кнопки «Купить» и цены в модалке |
| #337–#338 | Горизонтальный вылет страницы на десктопе; `overflow-x: clip` вместо `hidden` (иначе ломалась прилипающая шапка с поиском/VIN) |
| #333–#336 | Мега-меню «МАГАЗИН»: читаемый текст подкатегорий (правильные селекторы `.az-megamenu*`, перебит цвет прилипшей шапки) + высота под вьюпорт в обоих состояниях |
| #321–#332 | Hover «как у Kinoflex» в каталоге; сворачиваемые бренды в сайдбаре; единый вид карточек главной и каталога; фиксы обрезки в карусели и положения кнопки «В корзину»; баннеры-плитки; «стеклянная» прилипающая полоса |
| #320 | Актуализация всей документации (история PR #291–#319, раздел CSS-ловушек) |
...
| #193 | **Адаптивность по UX-аудиту (06.2026).** Тач-таргеты доведены до нормы: hero-CTA `min-height:48px`, кнопка «В корзину» `≥44px` + иконка избранного 44×44; поиск вынесен отдельной строкой на планшете (768–991px); боковые поля планшета 16→28px; иконки шапки на планшете 44×44; скрыт нативный скроллбар строки категорий; `scroll-margin-top:90px` под sticky-шапку; комфортный «подгляд» карточек на узких Android (≤360px). Контент: валюта приведена к смн (был ₽ в «бесплатной доставке»), локация/склад Москва→Худжанд во всех языках, fallback email/адрес в футере на бренд `.tj`. Проверено: поиск на телефоне и блог в 2 колонки на планшете уже работали — не трогали |
| #194 | **Чистка остатков шаблона по аудиту витрины/админки (06.2026).** Код-фиксы: опечатка «Похожие товары ~~товаров~~» (`catalog/part.php`); «В наличии (200 ~~Количество~~)» → «200 шт.» (новый ключ `pcs`); единицы измерения веса/габаритов — `кг`/`мм` (ключи `unit_kg`/`unit_mm`); иконка `fa-ruble`→`fa-money` у «Выручки» на дашборде; fallback телефона в шапке `+7`→`+992`; seed `site_email` в `schema.sql` → `.tj`. **Страница 403** в стиле Mazlay (`pages/403.php`, зеркало 404): нехватка прав теперь показывает аккуратный 403, а не редирект на слайдер (хелпер `denyAccess()`, задействован в `requireRole`+`requirePermission`). Проверено и НЕ требовало кода (уже сделано): Schema.org Product (JSON-LD), og-теги, `html lang`, `sitemap.php`, `robots.txt`, адаптивные admin-таблицы (`table-responsive`/`overflow-x`), валидация `redirect` и CSRF на входе. Остаётся (данные БД / отдельные фичи): значения соцсетей/email в `site_settings`, тексты слайдера, смена демо-паролей, гостевая корзина |
| #195 | **Большой пакет по аудиту: C2/C3/C5 + наполнение + онлайн-оплата (06.2026).** **C3 — единый макет персонала:** 8 страниц админки (`admin/products,sliders,banners`, `manager/pages,blog,reviews`, `superadmin/backup,manual`) переведены с витринного `header.php`/`footer.php` на `admin-header.php`/`admin-footer.php`; один бренд-логотип «<Сайт> Панель» + под-метка роли вместо трёх разных шапок («★СУПЕРАДМИН»/«ADMINPANEL»/«AUTOPARTS»). **C2 — защита от перебора пароля:** таблица `login_attempts`, лимит 5 неудач → блок на 15 мин по паре IP+логин; подключено к входу по email и по телефону (пароль/PIN). **C5 — ЧПУ для товаров:** красивые адреса `/product/123-slug` (правило в `.htaccess` + фолбэк-роутинг в `index.php` для nginx); `catalog/part.php` отдаёт `canonical` на ЧПУ и делает 301 со старых `?id=`; хелпер `partUrl()`; обновлены ~45 ссылок (витрина, корзина, избранное, поиск, мини-корзина, API); `sitemap.php` и JS-поиск отдают ЧПУ. **Наполнение каталога:** `seedDemoProducts()` — 42 редактируемых демо-товара по подкатегориям с фото и ценами (видны и правятся в админке). **Слайдер:** `seedSliderText()` — презентабельные заголовки/подзаголовки/кнопки вместо демо-текста («FIDANZA»…), только для нетронутых слайдов. **Онлайн-оплата со скидкой:** новый способ оплаты «Оплата онлайн» (вкл/выкл в `superadmin/settings.php`), скидка процентом или суммой и/или бесплатная доставка; чекаут считает и показывает скидку вживую; `orders.discount_amount`; скидка видна в деталях заказа покупателя. **Соцсети:** хелпер `$socUrl` и поля в настройках работают — значения вносит заказчик. Остаётся: смена демо-паролей (делает владелец), значения соцсетей/контактов в `site_settings`, выбор поставщика/локализации валюты (отложено) |
| #196 | **Редизайн страницы VIN-поиска `pages/vin.php` (06.2026).** Структура — по утверждённому макету (промо-баннер, карточка поиска с табами **«По VIN-коду» / «По параметрам»**, сетка популярных марок, «Как это работает», блок эксперта, trust-иконки, состояние «не найдено» с CTA), но **скин строго под наши токены** (красный `#C70909`, тёмный `#181a1f`, фон `#f4f5f7`), а не цвета макета. Полировка VIN-поля: счётчик `12/17`, кнопка неактивна до 17 валидных символов, кликабельный пример, тултип «где найти VIN»; иконки — инлайн-SVG (надёжнее «битых» шрифтовых). **«По параметрам»** — каскад Марка→Модель→Год из курируемого списка → поиск по каталогу + заявка эксперту (WhatsApp/Telegram с подставленными марка/модель/год), без выдуманных данных; задел под реальный TecDoc после покупки платного ключа. **Сохранён весь прежний функционал**: VIN-декодер + карточка авто с фото, совместимые из своего каталога + фильтр категорий + «в корзину» + инлайн-аналоги, **оригинальный каталог TecDoc по узлам + кроссы (мостик)**, история VIN. i18n: 33 ключа `vin_*` в ru/tg/en. Блок VIN на главной — не изменён |
| #197 | **Каталог: каркас сменных провайдеров (Этап 1 универсальной архитектуры).** Единый интерфейс `CatalogProvider` (`includes/catalog/Provider.php`) + фасад `Catalog::provider()` (`Manager.php`): фронт и эндпоинты работают через провайдера, не зная сервиса. Текущий PartsAPI обёрнут в `PartsApiAdapter` (поведение 1:1, нулевой риск). Новый `MockAdapter` — демо-каталог по VIN **без ключа** (узлы → детали → кроссы в нашем дизайне PR #196), чтобы показать работу до покупки подписки. В админке **«Суперадмин → VIN-поиск»** — выпадающий **«Провайдер каталога»** (PartsAPI / Демо), настройка `catalog_provider`; статус «Подключён» теперь от активного провайдера. Эндпоинты `api/vin_catalog.php`/`api/vin_crosses.php`, `pages/vin.php` переведены на `Catalog::provider()`. Дизайн VIN-страницы не менялся. План — `CATALOG_PLAN.md`. Дальше: Этап 2 (GenericRestAdapter + профили — смена REST-сервиса без кода), слой цен, Laximo, гостевая корзина |
| #202 | **VIN-каталог: редизайн под макет презентации (дерево узлов + карточки деталей).** Узлы теперь — **боковая панель «Узлы» + сетка карточек-узлов** (а не строка чипов): `.vin-tree`/`.vin-tree-side`/`.vin-node-card`, активный узел подсвечен, «Все узлы» — тёмная карточка. Детали — **карточки с фото-плейсхолдером, крупным красным ценником, строкой наличия/доставки и тёмной кнопкой «В корзину»** (`.vin-pcard`/`.vin-price`/`.vin-cart`), вместо таблицы. Кроссы разворачиваются под карточкой (`.vin-cross-box`). Ленивая подгрузка цен (склад→AutoEuro) превращает плашку «под заказ» в красный ценник + «поставщик · N дн». Всё в наших токенах (`#C70909`/`#181a1f`/`#f4f5f7`) — структура как у autodoc.ru (слайды 4–5), палитра наша. Бэкенд не тронут. Полный вид дерева (4+ узла) виден на провайдере «Демо» или при заполнении OEM-узлов |
| #201 | **Каталог: Laximo-адаптер — оригинальные OEM-каталоги (Этап 4, каркас).** `includes/catalog/LaximoAdapter.php` — код-адаптер (не профиль: Laximo это HMAC-шлюз `ec.api` + XML + цепочка токенов `ssd`). Реализованы реальная подпись `base64(md5(request·secret))`, POST через cURL, `findVehicleByVin`, рабочий `testConnection` (запрос `GetListCatalogs`). Доступы — `catalog_laximo_login`/`catalog_laximo_secret` (поля логин+секрет в админке). В выпадающем «Провайдер каталога» Laximo выбирается как любой провайдер. Полная выдача схем (ssd-цепочка) донастраивается на **боевом аккаунте Laximo**; до него адаптер gracefully не ломает сайт, а «Проверить соединение» на реальном ключе сразу покажет ответ. **Завершает все 5 этапов** универсального каталога |
| #200 | **Гостевая корзина — заказ без регистрации (Этап 5).** Снят login-wall: гость подбирает по VIN, кладёт в корзину и оформляет заказ без входа. Единое хранилище `includes/cart_lib.php`: гость → сессия (`$_SESSION['guest_cart']`), авторизованный → таблица `cart` (поведение 1:1). Мини-корзина в шапке (`getCartCount/getMiniCart/getMiniCartTotal`), `api/cart.php` (add/remove/update/count/mini), `buyer/cart.php`, `buyer/checkout.php` — все работают у гостя. При входе гостевая корзина **сливается** в БД (`cartMergeGuestIntoUser` из `loginUser()`). Заказ гостя привязывается к аккаунту **по телефону** (`guestOrderUserId` — находит/создаёт облегчённый аккаунт, как при регистрации по телефону), **без изменения схемы** `orders`; гость потом войдёт по SMS и увидит заказ. Миграций БД нет. Логика покрыта тестами (накопление, лимит 99, merge, find/create по телефону); поведение авторизованного покупателя не изменилось. **Этим завершены все 5 этапов** универсального каталога (см. `CATALOG_PLAN.md`) |
| #199 | **Каталог: слой цен — свой склад → AutoEuro → сомони (Этап 3).** Абстракция `PriceProvider` (`includes/catalog/PriceProvider.php`): каталог даёт OEM-номер — слой цен возвращает цену, независимо от провайдера каталога. `WarehousePriceProvider` — свой склад (приоритет); `AutoEuroPriceProvider` — фолбэк по OEM (дешёвое предложение с точным совпадением кода + общая наценка `global_markup`); `PriceAggregator` — склад→AutoEuro с кэшем `catalog_price_cache` (6 ч). Цены AutoEuro в RUB (база) → `formatPrice()` переводит в сомони, конвертация не нужна. Эндпоинт `api/vin_price.php` + **ленивая подгрузка** цен на странице VIN: детали «под заказ» (в каталоге и в кроссах) показывают цену + «склад/поставщик · N дн». Тумблер **«Цены из AutoEuro»** в админке (по умолчанию выкл → поведение как раньше: цена только со своего склада). Логика покрыта тестами (приоритет склада, гейтинг AutoEuro, graceful). Дальше: Laximo-адаптер, гостевая корзина |
| #198 | **Каталог: универсальный движок профилей (Этап 2) — подключение REST-сервиса БЕЗ кода.** `GenericRestAdapter` (`includes/catalog/GenericRestAdapter.php`) исполняет **профиль провайдера** (данные, не код): базовый URL, авторизация (`query`/`bearer`/`header`), шаблоны эндпоинтов с плейсхолдерами `{VIN}{KEY}{CAT}{ART}{BRAND}{TYPE}`, маппинг полей ответа. Два режима разбора: `objects` (массив деталей) и `pairs` (строка «БРЕНД|АРТ,…» как у PartsAPI). Реестр `CatalogProfiles` — встроенные пресеты + пользовательские из настройки `catalog_profiles` (JSON, валидируется). В админке — редактор **«Профили провайдеров (JSON)»**; добавленный профиль сразу появляется в выпадающем «Провайдер каталога». Чистые функции разбора (`buildUrl`/`getByPath`/`parseParts`) покрыты юнит-тестами (оба режима, urlencode, вложенные пути). Итог: купил ключ у любого REST/JSON-каталога → вставил профиль + ключ в админке → работает, **без программиста**. Дальше: слой цен (склад+AutoEuro→сомони), Laximo, гостевая корзина |
| #204 | **Каталог: Parts-Catalogs — визуальные OEM-схемы (кликабельные взрыв-схемы).** Код-адаптер `includes/catalog/PartsCatalogsAdapter.php` (как Laximo, но REST): авторизация `Authorization: <ключ>` (сырой), двухшаговая цепочка `VIN → car/info → {carId,catalogId,criteria} → groups2 (узлы) → parts2 (схема+детали)`. Формат сверен по официальному open-source клиенту `alex-ello/pc-client-slim`. `oemNodesForVin()` — реальное дерево узлов по авто; `schemeByVinCat()` — картинка схемы + хотспоты `[{n,x,y,w,h}]` + детали (с номером выноски). kv-кэш (`partsapi_kv_cache`, префикс `pc:`, 24ч на VIN), обогащение своим складом, graceful при лимите; `testConnection()` — реальный `GET /v1/catalogs/`. Новые эндпоинты `api/vin_nodes.php` (car-specific дерево) и `api/vin_scheme.php` (картинка+хотспоты+детали). Провайдер `partspc` в `Manager.php` + блок настроек «Parts-Catalogs» (ключ/база/таймаут/тумблер схем) в `superadmin/vin.php`. **Фронт** (`pages/vin.php`): панель схемы — «резиновый» `<img>` + абсолютно-позиционированные хотспоты (px→% при загрузке), **двусторонняя подсветка «точка ↔ карточка детали»** по номеру выноски; дерево узлов для PC грузится по VIN; общий билдер карточек `vinBuildPartsHtml`. Кроссов у PC нет → остаётся наш слой; цены — прежний слой (склад→AutoEuro→сомони). Изолированные неизвестные (формат авторизации, точные имена полей, CORS картинок) вынесены в один метод, при расхождении схема деградирует до «картинка+список» — 1-строчная правка на боевом ключе. Логика парсера проверена на моке. **После покупки:** Суперадмин → VIN → провайдер «Parts-Catalogs», ключ, «Проверить» → каталоги → схемы на странице VIN |
| #205–#206 | **Parts-Catalogs: переключаемая авторизация + фикс нечисловых ID.** `catalog_pc_auth` (header / Bearer / query `api_key`) и `catalog_pc_key_param` в админке — под разные варианты ключа Tradesoft. Фикс: ID узлов у PC — не числа (строки), это ломало схемы — поправлено во всех адаптерах и эндпоинтах (`Provider::searchByVinCat($vin, $cat)` — `$cat` теперь `string`) |
| #207 | **Каталог: реальный поиск «По параметрам»** (Марка→Модель→уточнения→авто) через Parts-Catalogs. Методы `pcBrands/pcModels/pcCarParams/pcCars` + `oemNodesForCar` в адаптере, эндпоинт `api/vin_params.php` (шаги `brands/models/carparams/cars`). Каскад в `pages/vin.php` |
| #208 | **Фикс UX:** авто-прокрутка к схеме при клике по узлу (не приходится листать вниз) |
| #209 | **Каталог в стиле 7zap:** русские названия (заголовок языка + `catalog_pc_lang`), **миниатюры схем** на карточках узлов (`img` из `groups2`), классификатор-сайдбар слева + смена контента **на месте** (сетка узлов ↔ вид узла), без прокрутки вниз |
| #210 | **Фикс:** «По параметрам» **самоактивируется** — каскад включается, если API реально вернул марки (не зависит от флага провайдера); иначе остаётся курируемый список |
| #211 | **Язык Parts-Catalogs:** отправляем `Accept-Language` + `Language` + query `lang` (best-effort). Если названия остаются английскими — язык переключается в аккаунте Tradesoft (у их менеджера). Выбор ru/en — `catalog_pc_lang` в админке |
| #213–#216 | **Полировка дизайна каталога:** схема на всю ширину колонки; контейнер VIN-страницы шире (`.vx .container` → `min(1560px,94vw)`); каталог/карточка авто/карточка поиска — на всю ширину (убрана узкая обёртка 900px); карточки узлов крупнее; **расшифровка сокращений** названий (`vinExpandAbbr`: К-т→Комплект, доосн.→дооснащения…); у деталей вместо пустого «фото» — **кликабельный номер-выноска** (`vinFocusPos` подсвечивает деталь на схеме; у OEM-каталога отдельных фото деталей нет — только взрыв-схема) |
| #217–#218 | **Документация:** развёрнутая карта кода `ARCHITECTURE.md` (все файлы/функции/эндпоинты/ключи `site_settings`/таблицы БД + индекс «хочу изменить X → файл Y»); актуализация README |
| #219 | **VIN-страница: порядок секций** — блок **«Оригинальный каталог по VIN»** поднят выше **«Совместимых запчастей»** (каталог виден сразу под карточкой авто). Блок совместимых перенесён целиком как самодостаточный фрагмент, PHP-условия/обёртки сбалансированы |
| #220 | **Карточка авто — реальные данные из Parts-Catalogs `car/info`.** Раньше показывались только Марка/Страна/Производитель (офлайн-разбор WMI): внешний VIN-декодер PartsAPI `VINdecodeOE` отвечает ошибкой `5007`, а методы `VINdecode`/`getCars` не входят в тариф ключа. Оказалось, провайдер **Parts-Catalogs** (тот же, что строит дерево узлов) через `car/info?q=<VIN>` **уже возвращает** модель, поколение, двигатель, год, кузов, регион, руль, коробку — но код в `vinToCar()` брал только `carId/brand`, остальное выбрасывал. Теперь `car/info` кэшируется одной записью (маршрутные поля + разобранные `attrs`); `vinToCar()` и новый публичный `vinCarInfo()` читают из **общего** кэша по VIN → доп. запросов к PC и лишних кредитов **нет** (тарификация PC — по VIN/24ч). Fallback на офлайн-разбор при пустом ответе |
| #221 | **VIN-каталог: двухколоночная раскладка узла** — картинка схемы слева `position:sticky` (остаётся на экране), список деталей скроллится справа; на мобильном (≤520px) схема залипает сверху. Режим включается только когда у узла есть картинка схемы |
| #222 | **VIN-схема: увеличение + подсказки.** Лайтбокс — схема на весь экран (зум колесом/кнопками ±, перетаскивание, закрытие `Esc`/фон/крестик); тултип с **именем детали + бренд + OEM** при наведении на выноску (счётчик остальных, если на позиции несколько); имя на схеме при наведении на карточку. Данные — из уже пришедшего ответа (карта «позиция→детали» на клиенте), доп. запросов нет |
| #223 | **VIN-схема: заметная подсветка выноски** — при наведении яркая заливка + внешнее свечение + лёгкая пульсация + центральная точка (тонкая рамка 2px терялась на плотном чертеже). Диагностика подтвердила: номера выносок и деталей совпадают 1:1, координаты валидные — проблема была в видимости |
| #224 | **VIN-схема: нормализация номеров + честный фолбэк.** `canonPos()` (`01`=`1`, срез пробелов) применяется к номерам выносок и деталей — связь работает независимо от формата номера в разных под-каталогах. Если у детали в данных PC **нет координаты точки** — при наведении показывается имя с пометкой «точка на схеме не указана» вместо «мёртвого» наведения. `CACHE_VER`↑ (сброс кэша схем) |
| #225 | **VIN-схема: «фото» карточки = вырезка из схемы.** Вместо пустого квадрата — увеличенный фрагмент взрыв-схемы вокруг точки детали (у OEM-каталога отдельных фото деталей нет — только общая схема). Координаты берутся из хотспотов, картинка — из уже загруженной схемы (`vinApplyCrops` считает `background-size/position`), доп. запросов нет; фолбэк для деталей без точки |
| #226–#228 | **VIN-схема: увеличение фрагмента.** Клик по вырезке-«фото» разворачивает **только фрагмент детали** на весь экран — масштаб подбирается так, чтобы окно вокруг детали заполнило ~90% экрана (лишнее обрезано). Зум считается **после готовности картинки** (иначе открывалась вся схема); убран тултип, который раздувался вместе с зумом |
| #229 | **VIN-схема: липкая схема под фиксированным меню** — `top` липкой схемы `14px`→`90px` + `max-height:calc(100vh-100px)`, чтобы при прокрутке списка деталей верх схемы не прятался за фиксированной шапкой сайта |
| #230 | **Документация:** changelog README — записи по PR #217–#229 |
| #231 | **VIN-каталог: фикс «пустого каталога».** `oemNodesForVin`/`oemNodesForCar` кэшировали результат по `carId` даже **пустой** — разовый сбой/лимит `groups2` → 0 узлов залипало на 24ч, и каталог «исчезал» у авто, где узлы есть (проявлялось в «По параметрам»). Теперь кэшируем **только непустой** результат. Диагностика: у Mercedes CLS — 120 узлов, страница показывала пусто из-за залипшего пустого кэша |
| #232 | **Карточка в режиме «По параметрам» — реальные атрибуты авто.** Раньше показывались только Марка/Модель (`car/info` только по VIN, по `carId` → 400). Теперь атрибуты берутся из объекта `cars2` (`parameters[]` + `description`): модель, модификация (Поколение), тип кузова, регион, а также год/двигатель/коробка если каталог их отдаёт; **Страна** — по бренду. `PartsCatalogsAdapter::pcCarAttrs()` + `parseCarAttrsFromCar()`; `pcCars` отдаёт `modelId`, ссылка выбора авто в каскаде передаёт `modelId` |
| #233 | **Новый дизайн карточки авто — вариант F (двухцветный сплит).** Слева тёмная панель (бейджи источник/кэш/VIN + чертёж-контур авто + марка/модель), справа белая колонка характеристик (только непустые поля). Пустой красный «силуэт»/фото заменены чертежом. Карточка общая для VIN и «По параметрам» |
| #234–#235 | **Полировка карточки F по отзыву:** чертёж авто меньше + полупрозрачный вордмарк марки («лого»-эффект); характеристики читаемее (темнее подписи, крупнее значения); характеристики в **две колонки** + левая панель уже — карточка ниже и компактнее; адаптив (≤760px — одна колонка) |
| #257 | **Боевой SMS-шлюз OsonSMS.** `sendSms()` диспетчерит тест-режим/`osonSmsSend()` по настройке `sms_provider`; новый `osonSmsLocalPhone()` приводит номер к локальному 9-значному формату, которого требует OsonSMS. Карточка «SMS-шлюз» в `superadmin/settings.php` (логин/хэш/отправитель/сервер + кнопка «Отправить тест»), реквизиты только в `site_settings` |
| #258 | **Фикс:** Enter в поле телефона на входе/регистрации запускал преждевременную отправку формы (ошибка «Введите код из SMS или пароль» до получения кода) — теперь запускает «Получить код», если код ещё не показан |
| #259 | **Фикс:** кнопка «Войти» на форме телефона путала клиентов, будучи видимой сразу рядом с «Получить код» — теперь скрыта (`phone_login_submit`) до получения SMS-кода, входа по паролю или PIN (`updatePhoneSubmitVisibility()`) |
| #260 | **Фикс:** ссылка «Вход для сотрудников (PIN)» наезжала на «Войти по паролю» на одну строку — плавающая `.login_submit a` темы Mazlay без clearfix; добавлен `.login_submit::after { clear:both }` |
| #267 | **Чат покупатель ↔ менеджер на сайте.** Таблица `messages`, ветка = пара (покупатель, заказ): `order_id=NULL` — общая поддержка, число — переписка по заказу. У покупателя страница `buyer/messages.php` (список веток + отправка), у сотрудника `admin/messages.php` (входящие, непрочитанные выше). Значок непрочитанного в меню кабинета и в сайдбаре админки. Автосообщение-«чек» (номер/состав/сумма/оплата) падает в переписку заказа при оформлении (`buyer/checkout.php`). Логика — `includes/messaging.php` (graceful при недоступности БД) |
| #268 | **Заказ: статусы в чат + заявка поставщику (ветка «человек в Москве»).** Смена статуса заказа в `admin/orders.php` шлёт автосообщение покупателю в переписку заказа (подтверждён/отправлен/доставлен/отменён). На карточке заказа — блок «Заявка поставщику»: готовый текст со списком деталей (бренд/артикул/кол-во, покупатель, доставка), кнопка «Скопировать» — менеджер отправляет человеку в Москве в мессенджере. Денег не двигает. Ветка «напрямую в AutoEuro» (реальные деньги) — отдельным шагом, под явным подтверждением |
| #263–#264 | **Прогрев библиотеки каталога — CLI-скрипт `superadmin/catalog_library_seed.php`.** В отличие от кнопки/cron (только добирают схемы у уже известных авто), скрипт сам открывает новые марки/модели/авто через `pcBrands()/pcModels()/pcCars()` (без VIN, «по параметрам») и сохраняет их в `catalog_library_*`. Обход марок **по кругу** (round-robin) — разнообразие сразу; `schemes_per_car=-1` — собрать все схемы авто. Параметры запуска — `budget_cars/schemes_per_car/pause_ms`. ⚠️ Каждое новое авто — платный запрос к Parts-Catalogs; запускать осознанно (не по расписанию) |
| #265–#266 | **AutoEuro: лог несовпадений + маппинг брендов.** Когда цена у AutoEuro не находится — запись `autoeuro_price_miss` в `warehouse_api_log` (видно в Суперадмин → Склад → Лог). Настройка `autoeuro_brand_map` (строки «ИСХОДНЫЙ = Замена») подменяет бренд перед запросом, если каталог и AutoEuro называют его по-разному |
| #269–#277 | **AutoEuro — цены на витрине заработали (боевой разбор).** Ключевые фиксы: `with_offers=1` (без него API отдаёт пусто), резолв каноничных бренд+код через `search_brands` перед `search_items` (`AutoEuro::searchItemsSmart` — AutoEuro хранит код в своём формате, напр. с пробелами `1 987 301 001`), выбор **московского** пункта доставки (кнопка «Выбрать» вместо ручного копирования 90-символьного ключа + чистка пробелов; `delivery` агрегирует все склады), отображение `«под заказ» + ожидаемая дата` при `stock=0` (янтарный ценник вместо красного), фикс HTML-значка валюты. Диагностика `api/autoeuro_search.php?raw=1&method=…` (`AutoEuro::debugRaw`) |
| #278–#280 | **AutoEuro — валюта и наценка.** Цены приходят в рублях → перевод в сомони по `autoeuro_rub_rate` (сомони за 1 рубль, поле в админке; иначе показывались 1:1, ×9). Отдельная наценка `autoeuro_markup` на товары поставщика (пусто → общая `global_markup`). Разделитель тысяч — пробел (`1 474.98 смн`) |
| #282–#283 | **Контакты: телефон и авто-формат.** Номер по умолчанию `+992 92 612-22-22`; `formatPhone()` приводит любой ввод к виду `+992 XX XXX-XX-XX`, `phoneTel()` — цифры для ссылки звонка. Владелец вводит номер как удобно — на сайте показывается аккуратно |
| #291 | **Аудит документации.** Полная сверка `.md` с кодом: все эндпоинты/файлы/настройки покрыты; в шапку README добавлена таблица «Какой документ для чего», CATALOG_PLAN исправлен (было «к разработке не приступали» — стало «реализовано, работает боевым»), ARCHITECTURE дополнен блоком про `offersByOem()`/доставку Худжанд/модалку заявки, CHANGES помечен как исторический одноразовый документ (не дополняется). Ключей `map_lat/lng/zoom`, `review_msg_*` в доках не было — оказалось, они уже задокументированы сокращённо (ложное срабатывание аудита) |
| #292–#306 | **Мобильная адаптация шапки, поиска, карточек и модалки (15 итераций).** Форма VIN на мобильном переворачивалась (тема ставит `flex-direction: column-reverse`) — вернул нормальный ряд, шрифт 16px чтоб iOS не зумил. Модалка выбора доставки выводилась голым блоком без стилей — её CSS лежал в `<style>` внутри `if($searchPerfomed)` (HTML вне условия), перенёс в безусловный блок. Шапка на телефоне переливалась за экран (`.row { margin: -8px }` + `width: 100%`); ужаты отступы главной, единый стиль иконок (призрачные квадраты 42×42 вместо смеси красных плашек и голых стрелок), компактный логотип (52→34px), поле VIN сжималось в высокий белый блок (базовое `flex: 0 1 210px` работало как высота в колонке — сбросил flex+фикс. высота 46px). **#305:** мобильный поиск переделан в **сегментированный переключатель «Запчасти / По VIN»** (одно поле за раз вместо двух подряд). **#302–#303:** карточки главной выровнены — колонка на всю высоту, кнопка через `margin-top: auto`; логотип не в углу — иконки/поиск переносятся через `display: contents` |
| #293 | **Витринные тексты: наш склад в Худжанде, а не в Москве.** `lang/ru.php`, `lang/tg.php` — в трёх фразах (`about_desc`, `our_history_text`, `fast_delivery_text`): Москва → Худжанд, Россия → Таджикистан. В английском (`lang/en.php`) уже было правильно. Логику AutoEuro (Москва как реальный пункт получения) намеренно не трогал |
| #294–#296 | **Пустота между блоками главной.** У категорий `mb-80` (80px) + `home_section_bg { padding-top: 75px }` = ~160px «дыры» перед «Магазин / Рекомендуемые товары». Ужато до 10/26px десктоп, 8/22px планшет, 6/18px телефон (`!important` + обнуление возможных отступов внутренних обёрток) |
| #297 | **Читаемый выпадающий список языков.** Пункты «RU/TJ/EN» почти не видны — правило тёмной шапки `.header_top a { color: #e8e8e8 !important }` красило все ссылки, а панель на белом фоне. Возвращён тёмный цвет `#222` именно внутри `.dropdown_language/_currency/_links`, при наведении красный акцент |
| #298 | **UTF-8 для CSS/JS.** Открытие `custom.css` в браузере показывало русские комментарии кракозябрами. Добавлены `@charset "UTF-8";` первой строкой + `AddCharset UTF-8 .css .js` в `.htaccess` (HTTP-заголовок имеет приоритет над `@charset`). На работу стилей не влияло — только на читаемость файла |
| #307 | **Промо-карточка «Товары со скидкой» — перелив и пустота.** Свои inline-стили с `flex-wrap: wrap` конфликтовали с общим правилом `.single_product { flex-direction: column }` (PR #302) — содержимое раскладывалось в несколько колонок и уезжало за границы. Дан отдельный класс `.promo_deal`, раскладка задана явно: фото слева / текст справа, `nowrap`, `min-width: 0` у текста. На телефоне (≤767px) — фото сверху, текст под ним |
| #308 | **Двойное содержимое карточек каталога + login-wall у «В корзину».** У карточки два блока `.grid_content` и `.list_content` (для видов сеткой/списком), тема скрывает лишний правилом с одинаковой специфичностью — мой `custom.css` грузится позже и перебивал скрытие: бренд/цена/кнопка **дублировались**. Правило `.single_product .product_content { display: flex }` теперь исключает оба варианта (`:not(.grid_content):not(.list_content)`). Плюс убран забытый login-wall: гостю в каталоге показывалось «ВОЙТИ» вместо «В КОРЗИНУ», хотя гостевая корзина работает (`cart_lib` поддерживает гостя) |
| #309–#319 | **Оформление карточек каталога и полей поиска — 11 итераций.** Плитки с рамкой/тенью/промежутками (`.shop_wrapper .single_product`), постоянная тень (не только по наведению), кнопка «В корзину» **всегда видна** в потоке (у темы `position: absolute; bottom: -55px` — на тач-экранах недоступно и обрезалось `overflow: hidden`), сердечко избранного вынесено на фото плавающей иконкой (не занимает место рядом с кнопкой), плавный зум фото при наведении, убрана иконка «глаз» (быстрый просмотр — как всплывающее окошко). Типографика на Rubik: бренд — мелкий серый uppercase, название — 2 строки с `line-clamp`, цена **всегда в одну строку** (старая+новая в ряд, а не столбиком — это и делало карточки со скидкой выше соседних). Значок «-N%» и сердечко разведены по углам. Кнопка «В корзину» центрирована (flex + height 46px). Сайдбар фильтров — карточка с тенью + `position: sticky` на десктопе. Поле VIN на десктопе: 50px высота как у поля запчастей (было schлопывалось до высоты текста), уширено до 300px, плейсхолдер не обрезается. Поиск в шапке — **два раздельных белых блока** с промежутком и тенью (первую попытку слить в единую полосу владелец отменил). Sticky-поиск при прокрутке — тот же вид |
| #290 | **Прилипающий поиск при прокрутке + отдельное поле VIN.** В красную «прилипающую» полосу (`.sticky-header.sticky` — тема ставит `.sticky` при скролле >100px, новый JS не нужен) добавлен компактный поиск. Вверху рядом с обычным поиском — отдельное поле «VIN — 17 символов». Стили `.sticky_search` в `custom.css` |
| #288–#289 | **Диагностика VIN + фикс кэша каскада «По параметрам».** (#288) Тест соединения Parts-Catalogs при нераспознанном VIN теперь показывает сырой ответ именно от `v1/car/info/` — видно причину (нет в базе / тариф), а не только «не распознан». (#289) Каскад «По параметрам» показывал короткий заглушечный список вместо 74 брендов: пустой список брендов однажды закэшировался и «залипал» на 30 дней. Теперь пустой кэш = промах (перезапрос вживую), пустое не кэшируется — самовосстановление без ручной чистки |
| #287 | **UI:** убран ценник из карточки детали поставщика — осталась только кнопка «Купить» (цены и сроки видны в модалке при выборе доставки) |
| #286 | **UI: кнопка «Купить» + модалка выбора доставки (бланк заявки).** Вместо тесного ценника со списком радио — карточка показывает цену «от …» и кнопку **«Купить»**; по клику открывается модалка (бланк) с чистым выбором вариантов доставки, количеством и комментарием. «Оформить заявку» → `api/vin_order_request.php`: заявка уходит менеджеру в переписку (`messages`, ветка поддержки), покупателю — подтверждение. Требуется вход (иначе редирект на логин). Детали AutoEuro нет в каталоге/корзине — заявку менеджер обрабатывает вручную (заказ у поставщика); автозаказ/оплата — отдельный шаг |
| #285 | **AutoEuro — умный выбор предложения + доставка Москва→Худжанд + выбор варианта.** У поставщика на артикул много предложений (разные склады): в наличии → самое дешёвое из наличия, иначе → самое быстрое под заказ (`offersByOem()`, `priceByOem()` — лучший из списка). В цене AutoEuro зашита доставка только **до Москвы**, поэтому логистику Москва→Худжанд добавляем сами: тумблер `autoeuro_khj_enabled`, надбавка суммой/процентом (`autoeuro_khj_mode`/`autoeuro_khj_value`) и дни к сроку (`autoeuro_khj_days`) — покупателю показывается итог до Худжанда. На витрине покупатель выбирает из нескольких вариантов «быстрее-дороже / дешевле-дольше» (`autoeuro_offers_limit`); тумблер показа срока `autoeuro_offer_mode` A (итог) / B (разбивка) |

> **Стадия:** все PR #219–#235 **влиты в `main`**; развёртывание на боевом сервере — `git pull` в `~/public_html`. Правки не трогают секреты (ключи API/пароли хранятся только в БД `site_settings`, в репозиторий не попадают).
>
> **Проверено в бою:** VIN-режим и **«По параметрам»** дают одинаковый каталог (узлы → схемы → детали → вырезки), карточка авто заполняется реальными атрибутами.
>
> **В работе (по желанию):** для отдельных авто без данных у Parts-Catalogs (`oemNodesForCar` = 0 узлов, напр. спецтехника) — понятное сообщение «оригинальный каталог для этого авто недоступен» вместо пустого блока.

---

### 📌 Что осталось (по состоянию на 08.2026)

**Ждём ответа от поставщика (блокирует работу):**
- 🔴 **Tradesoft / Parts-Catalogs: что именно считается «запросом» по тарифу.** Израсходовано
  999 из 1000, сервис отключается автоматически до 1-го числа. Прежний вывод «обход дерева
  узлов и сбор схем тариф не тратят» **опровергнут** письмом поставщика, поэтому весь
  пакетный сбор каталога остановлен. Витрина живёт из библиотеки. До ответа не запускать
  `catalog_library_rebuild.php` и `catalog_library_seed.php`.

**Отложено решением заказчика:**
- **Сквозная проверка Фаз 2 и 3a на боевом**: заказ с товарами двух продавцов → подзаказы
  с комиссией → статус «Доставлен» → балансы → выплата. Материал для неё готов —
  `php sql/seed_test_sellers.php` заводит магазины с товарами.
- Перевыпуск ключа Parts-Catalogs — до полноценного запуска. Ключ в git отсутствует
  (проверена вся история), живёт в `site_settings` и в `~/.bash_history` вне `public_html`.

**Делает владелец / заказчик (не код):**
- 🔴 Сменить демо-пароль `Password123!` на всех боевых аккаунтах через `superadmin/users.php` (из соображений безопасности это не делается из песочницы/гита).
- Заполнить реальные значения соцсетей и контактов в `superadmin/settings.php` (Telegram/WhatsApp/Instagram/Facebook, email, адрес). Вывод уже работает — иконка соцсети показывается только при заполненном значении.
- При желании — заменить слайды/тексты слайдера и дозаполнить каталог своими товарами (демо-товары можно редактировать и удалять в админке).

**Отложенные бизнес-решения:**
- Поставщик и локализация валюты: AutoEuro + рубли против локального поставщика в Худжанде + сомони (по просьбе заказчика отложено).

**Сделано за 07.2026:**
- ✅ **Цены AutoEuro на витрине** — деталь не со склада показывает цену/наличие/срок от AutoEuro (перевод в сомони по курсу `autoeuro_rub_rate` + наценка `autoeuro_markup`). Настройка — Суперадмин → Склад; включение — Суперадмин → VIN → «Цены из AutoEuro».
- ✅ **Чат покупатель↔менеджер** + автосообщения по статусу заказа + «Заявка поставщику».
- ✅ **Боевой SMS-шлюз OsonSMS**; авто-формат телефона.

**Сделано за 08.2026:**
- ✅ **Поиск у поставщика** — по артикулу (живой AutoEuro) и по названию/бренду (свой словарь
  из официального прайса, 229k позиций) карточками с фото + «Показать ещё».
- ✅ **Маркетплейс, Фаза 1** — продавцы, их товары, модерация, витрина магазина.
- ✅ **Маркетплейс, Фаза 2** — заказы с разбивкой по продавцам, кабинет заказов продавца,
  комиссия и выплаты в админке.
- ✅ **Полнота каталога** — зашитый лимит 120 узлов заменён настройками (реально у машин
  229…1000 узлов, то есть больше половины каталога было невидимо).
- ✅ **Инструменты обслуживания библиотеки** + шесть починенных граблей (обрывы соединения,
  потеря деревьев, вечное выпадение машины из очереди, параллельные прогоны).

**Сделано за 08.2026 (продолжение):**
- ✅ **Buy-box** — конкуренция продавцов на одной карточке, выбор лучшего предложения,
  варианты товара (цвет/размер/возраст), тестовые продавцы для проверки.

**Возможные следующие фичи (код):**
- **Варианты на общей карточке** (полная модель Ozon) — снимет разрыв линейки, когда
  карточку выигрывает конкурент. Сейчас варианты принадлежат продавцу.
- **Свой счётчик запросов к Parts-Catalogs** (отдельно VIN / `groups2` / `parts2`) — чтобы
  не зависеть от того, что квоту показывает только поставщик.
- **Автоматическое обновление протухшего токена комплектации** — сейчас лечится только
  ручным перезапуском декодирования VIN.
- **Автозаказ у AutoEuro (`create_order`) — денежная ветка.** Сейчас закупка у поставщика оформляется вручную («Заявка поставщику» на карточке заказа). Автоматический заказ списывает деньги со счёта AutoEuro — подключается отдельно, под явным решением и с тестом (нужен `payer_key` + баланс).
- **Реальный платёжный шлюз (онлайн-оплата картой/QR).** Сейчас «Оплата онлайн» фиксирует способ и применяет скидку, а приём платежа требует договора эквайринга с банком Таджикистана и интеграции его шлюза.
- **Аналоги-кроссы с ценой** — если точной детали нет у AutoEuro, но есть заменители, можно показывать их с ценой (сейчас на витрине цена только при точном совпадении бренд+артикул).
- Полная локализация ru/tg/en страницы VIN: каркас переведён через `t()`, но часть текста в блоке результатов (карточка авто, оригинальный каталог, схемы) пока на русском.
- Доставка в дополнительные страны — включить нужные страны в настройках телефона и дополнить список зон доставки.
- SEO-страницы по конкретным авто и расширенная аналитика спроса — по мере накопления данных библиотеки каталога (см. §8 клиентского PDF).

---

### Как работает: страница VIN-поиска (`pages/vin.php`)

Структура сверху вниз: промо-баннер → карточка поиска с двумя табами → (результаты при поиске) → популярные марки → «Как это работает» → блок эксперта → trust-иконки. Скин задаётся scoped-классами `.vx*` с CSS-переменными бренда (`--vx-red:#C70909`, `--vx-ink:#181a1f`, `--vx-bg:#f4f5f7`) — структура как в макете, цвета наши.

**Два пути подбора:**
- **По VIN** — GET-форма (`?vin=…`). Сервер: `VinService::validate()` → `VinService::decode()` (PartsAPI `VINdecodeOE` + NHTSA, с кэшем). Дальше: карточка авто, совместимые из своего каталога (`searchCompatibleParts` + фасеты категорий), и **оригинальный каталог TecDoc** — грузится асинхронно по узлам (`api/vin_catalog.php` → `CatalogApi::searchByVinCat`, один узел = один запрос, бережёт лимит ключа), с «мостиком» кроссов (`api/vin_crosses.php` → `getCrosses` → сверка со складом).
- **По параметрам** — клиентский каскад из `$vinMakes` (курируемый список) → «Подобрать» открывает блок с переходом в поиск каталога и заявкой эксперту (WhatsApp/Telegram c подставленными марка/модель/год). Реальный TecDoc-подбор подключается после платного ключа (см. выше).

**Деградация на демо-ключе:** при превышении суточного лимита PartsAPI каталог/кроссы показывают понятное сообщение и не «жгут» квоту; VIN-декод и свой каталог продолжают работать. На платном ключе всё работает в полном объёме без изменений кода.

---

### Как работает: Parts-Catalogs — визуальные OEM-схемы (PR #204)

**Провайдер каталога сменный** (`includes/catalog/`): фронт и эндпоинты работают через `Catalog::provider()`, не зная сервиса. Выбор — в админке (`catalog_provider`). Для Parts-Catalogs выбран отдельный код-адаптер (REST с токеном `criteria`, протянутым между вызовами) — по образцу их официального клиента `alex-ello/pc-client-slim`.

**Цепочка (adapter):**
1. `GET v1/car/info/?q=VIN` → `{carId, catalogId, criteria}` (кэш `pc:car:{VIN}`, 24ч).
2. `GET v1/catalogs/{catalogId}/groups2?carId&criteria` → дерево узлов; листья `hasParts` → узлы (`api/vin_nodes.php`, кэш `pc:nodes:…`).
3. Клик по узлу → `GET v1/catalogs/{catalogId}/parts2?carId&groupId&criteria` → `{img, positions[], partGroups[]}` (`api/vin_scheme.php`, кэш `pc:parts:…`).

**Визуальная схема (front, `pages/vin.php`):** картинка `img` в «резиновый» `<img>`; каждый `positions[i]` = `<a class="vin-hot">` с координатами `[x,y,w,h]`, пересчитанными px→% при загрузке картинки; наведение/клик по номеру подсвечивает карточку детали и наоборот (связь — `positions[].number == part.positionNumber`). Детали рендерятся общим `vinBuildPartsHtml` (фото-плейсхолдер, цена, «в корзину», кроссы, ленивые цены).

**Цена/кроссы:** у Parts-Catalogs своих цен и кроссов нет → цена берётся прежним слоем (свой склад → AutoEuro → сомони), кроссы — существующим слоем. **Тарификация PC** — по VIN/24ч, поэтому кэшируем по VIN на 24ч (повторные просмотры одной машины бесплатны).

**Авторизация переключается в админке** (без кода): «Способ авторизации» = заголовок `Authorization: <ключ>` / `Authorization: Bearer <ключ>` / параметр запроса `?api_key=<ключ>` (имя параметра настраивается — `api_key` или `auth_key`), плюс базовый URL (у Tradesoft бывает хост с `/api/`). Tradesoft — оператор Parts-Catalogs; его ключ обычно передаётся параметром — выбирается в списке, «Проверить соединение» покажет результат.

**Что подтверждается на боевом ключе** (изолировано, деградация без поломок): рабочий способ авторизации (тумблер выше), точные имена полей схемы (парсер `fetchScheme()`), кросс-доменная отдача картинок (при блокировке — добавить мини-прокси).
...
**SMS-шлюз — боевая отправка через OsonSMS (PR #257).** `sendSms()` в `includes/functions.php` диспетчерит по настройке `sms_provider`: пусто → тест-режим (код на экране + `storage/sms.log`); `osonsms` → реальная отправка через `osonSmsSend()` (GET на `sendsms_v1.php`, `Authorization: Bearer <hash>`, номер в локальном 9-значном формате через `osonSmsLocalPhone()`). Настройки — карточка **«SMS-шлюз»** в `superadmin/settings.php` (логин/хэш/имя отправителя/сервер + кнопка «Отправить тест»); реквизиты хранятся только в `site_settings`, в репозиторий не коммитятся.

**UX-фиксы формы входа/регистрации по телефону (PR #258–#260):**
- Enter в поле телефона запускает «Получить код», а не преждевременную отправку формы (до этого показывалась путающая ошибка «Введите код из SMS или пароль»).
- Кнопка **«Войти»** на форме телефона скрыта до тех пор, пока не получен код, не открыт вход по паролю или PIN (`phone_login_submit` + `updatePhoneSubmitVisibility()` в `assets/js/app.js`) — раньше была видна сразу и путала клиентов.
- Clearfix `.login_submit::after` — устраняет наезд ссылки «Вход для сотрудников (PIN)» на «Войти по паролю» (следствие плавающей `.login_submit a` в теме Mazlay).
...
### 🛒 Цены поставщика AutoEuro на витрине — PR #265–#280

**Что это.** Для деталей, которых нет на своём складе (например, в подборе по VIN),
сайт показывает **цену, наличие и срок** от поставщика **AutoEuro** — с переводом в
сомони и вашей наценкой. Заказ у покупателя оформляется как обычно, а закупку у
поставщика менеджер делает вручную (кнопка «Заявка поставщику» на карточке заказа).

**Как подключить (Суперадмин → Склад):**
1. **API-ключ** из личного кабинета shop.autoeuro.ru → Настройки → API. Поставить
   галочку «Интеграция активна», сохранить. Кнопка «Баланс» проверяет ключ.
2. **Ключ доставки** — нажать «Варианты доставки», у нужного пункта нажать
   **«Выбрать»** (ключ сам встанет в поле). Брать **московский** пункт — он
   агрегирует все склады. Сохранить. *(`payer_key` нужен только для автозаказа —
   пропустить.)*
3. **Курс: сомони за 1 рубль** — вписать актуальный (напр. `0.11`). Цены AutoEuro
   в рублях, переводятся в сомони по этому курсу. Держать актуальным.
4. **Наценка на товары AutoEuro (%)** — своя наценка на детали поставщика
   (пусто → общая `global_markup`).
5. **Доставка Москва → Худжанд** — в цене AutoEuro заложена доставка только
   **до Москвы**. Наша логистика Москва→Худжанд добавляется сверху: тумблер
   **«Добавлять доставку Москва → Худжанд»**, режим надбавки **сумма (сомони)**
   или **процент (%)** + значение, и **срок в днях**. Покупателю показывается
   уже итог до Худжанда. Выключить — если заказчик возит сам.
6. **Показ покупателю.** У поставщика на один артикул много предложений (разные
   склады) — покупатель видит **несколько вариантов** «быстрее-дороже /
   дешевле-дольше» и выбирает сам (число вариантов — настройка, деф. 3). Тумблер
   **A/B** переключает показ срока: **A** — только итог (напр. «до 05.10.2026»),
   **B** — с разбивкой (Москва + N дн → Худжанд).
7. **Суперадмин → VIN-поиск** → галочка **«Цены из AutoEuro для деталей не со
   склада»** → сохранить. *(Без этого шага цены на витрине не появятся.)*

**Как считается цена:** `закупка(руб) × курс(сомони/рубль) × (1 + наценка%)`,
затем **+ надбавка Москва→Худжанд** (сумма или %). Срок = дата AutoEuro (прибытие
в Москву) **+ дни Москва→Худжанд**. В наличии — самое дешёвое из наличия; если
наличия нет — самое быстрое под заказ.

**Технические детали.** `AutoEuroPriceProvider::searchItemsSmart()` сначала спрашивает
`search_brands` (резолвит каноничные бренд+код — AutoEuro хранит код в своём формате,
напр. с пробелами `1 987 301 001`), затем `search_items` с `with_offers=1` (иначе цен
нет), берёт **самое дешёвое** предложение с точным совпадением кода. Если бренд у
каталога и AutoEuro называется по-разному — пара в поле «Соответствие названий
брендов» (`autoeuro_brand_map`); несовпадения копятся в логе `autoeuro_price_miss`.
При `stock=0` цена показывается янтарным блоком «под заказ · ожид. ДД.ММ.ГГГГ».
Диагностика сырого ответа: `api/autoeuro_search.php?...&raw=1[&method=search_brands|get_deliveries]`.

> ⚠️ Автозаказ у поставщика (`create_order`, списание со счёта AutoEuro) **не
> подключён** — только показ цен. Закупка оформляется вручную через «Заявку
> поставщику» на карточке заказа.

---

### 💬 Чат покупатель ↔ менеджер — PR #267, #268

Переписка на сайте: общая **поддержка** + отдельная **ветка по каждому заказу**.
Логика — `includes/messaging.php` (таблица `messages` создаётся сама). У покупателя —
раздел **«Сообщения»** в кабинете (`buyer/messages.php`), у сотрудника — **«Сообщения»**
в админке (`admin/messages.php`, непрочитанные сверху). Значок непрочитанного в обоих
меню. **Автосообщения** от системы: «чек» при оформлении заказа (номер, состав, сумма,
оплата) и уведомления при смене статуса заказа (подтверждён/отправлен/доставлен/отменён).
...
- [x] **Боевой SMS-шлюз** — подключён OsonSMS (`sendSms()` → `osonSmsSend()`), настраивается в **Суперадмин → Настройки → SMS-шлюз** (логин/хэш/отправитель + тест-кнопка). Тест-режим (код на экране) остаётся, если провайдер не выбран.
...
- [x] **Цены каталога от AutoEuro** — подключены: деталь не со склада показывает цену/наличие/срок AutoEuro (RUB→сомони по курсу `autoeuro_rub_rate` + наценка `autoeuro_markup`). Курс рубля держать актуальным в Суперадмин → Склад.
- [ ] **Автозаказ у AutoEuro (`create_order`)** — денежная ветка; сейчас закупка вручную через «Заявку поставщику». Подключать под явным решением (нужен `payer_key` + баланс + тест).
- [ ] **Онлайн-оплата картой/QR** — сейчас фиксируется только способ оплаты и скидка; для приёма реальных платежей нужен договор эквайринга с банком Таджикистана и интеграция его шлюза. **Это же блокирует Фазу 3 маркетплейса** (комиссия и выплаты продавцам).
- [x] **Поиск по поставщику AutoEuro** — по **артикулу** (живой запрос) и по **названию/бренду** (словарь `ae_part_dictionary`, наполняется импортом прайс-листа). Цены всегда живые. См. раздел «Поиск по поставщику».
- [x] **Маркетплейс, Фаза 1** — продавцы регистрируются, выкладывают товары, админ модерирует, товар выходит в каталог с меткой «Продавец». См. раздел «Маркетплейс».
- [ ] **Маркетплейс, Фаза 2** — заказы с разбивкой по продавцам + кабинет заказов продавца + buy-box (один артикул у нескольких продавцов; сейчас `parts.part_number` UNIQUE).
- [ ] **Обновление словаря названий по расписанию** — прайс генерируется в кабинете AutoEuro ежедневно; можно повесить cron на скачивание + `ae_dict_import_csv.php`.
- [ ] **Фото деталей поставщика** — ни API, ни прайс AutoEuro фото не содержат. Варианты: лицензия TecDoc или переход на поставщика со встроенным каталогом (Emex/Autopiter).
...
## 🔎 Поиск по поставщику AutoEuro: по артикулу и по названию

Обычный поиск в шапке ищет **не только по нашему каталогу**, но и у поставщика.
Результаты поставщика выводятся отдельным блоком **«Найдено у поставщика AutoEuro»**
карточками с фото, ценой и кнопкой «Купить».

### Два режима поиска

| Что ввели | Что происходит |
|---|---|
| **Артикул** (`0451103316`, `76876`, `W712/75`) | живой запрос к AutoEuro: `search_brands(код)` → по каждому бренду `search_items` → карточка на бренд с готовой ценой |
| **Название или бренд** («ремень грм», «BOSCH») | поиск по **нашему словарю** `ae_part_dictionary`; цена каждой карточки подгружается **живьём** из API |

Детектор в `search/index.php`: «похоже на артикул» = есть цифра, нет кириллицы, 3–40 символов.

### Почему нужен словарь названий
**API AutoEuro ищет только по артикулу.** По слову «масло» или по бренду «BOSCH» он не
вернёт ничего — метода «все товары бренда» в API тоже нет. Поэтому **названия храним у
себя**, а цену всегда берём живую (в словаре цен нет).

### Как наполнить словарь (правильный способ)

1. **Личный кабинет AutoEuro → Настройки → Прайс-лист**: формат **CSV**, кодировка **UTF-8**,
   e-mail «не отправлять», нажать **«Обновить прайс-лист»** и дождаться готовности.
2. Скачать файл и положить на сервер (например `~/public_html/PRICE.csv`).
3. Импортировать:
```bash
cd ~/public_html
php superadmin/ae_dict_import_csv.php PRICE.csv          # импорт
php superadmin/ae_dict_import_csv.php PRICE.csv --dry    # только проверка разбора
```
Скрипт берёт `Производитель → бренд`, `НомерПроизводителя → артикул`, `Применение → название`,
пакетно заливает в `ae_part_dictionary` и создаёт `FULLTEXT(name)`.
**Ноль запросов к API.** Проверено на боевом прайсе: 229 129 строк → 229 127 названий.

Проверка результата:
```bash
php -r 'require "config/config.php"; require "includes/catalog/AutoEuroPriceProvider.php";
 foreach (AutoEuroPriceProvider::dictionarySearch("ремень грм", 5) as $r)
   echo $r["oem"]." | ".$r["brand"]." | ".$r["name"]."\n";'
```

### ⚠️ Чего делать НЕЛЬЗЯ
- **Перебирать API миллионами запросов** и **парсить сайт autoeuro.ru** — это нарушение
  условий и **бан вашего рабочего аккаунта поставщика**. Плюс списка всех артикулов у вас
  всё равно нет. Массовое наполнение — только через прайс-лист.
- Запасной путь (если прайса нет): `php superadmin/ae_dict_batch.php 100 500` — добор
  названий по артикулам, которые **уже есть в вашей БД**, порциями с паузой.

### Фото
В API и в прайсе AutoEuro **фотографий нет**. На карточках поставщика показывается наше
фото (если такой артикул есть в каталоге), иначе аккуратная заглушка. Реальные фото по
всему рынку = каталог TecDoc-класса (платная лицензия) или поставщик со встроенным
каталогом (Emex/Autopiter — там поиск по названию и фото «из коробки», API бесплатный
для клиента).

### Кнопка «Показать ещё»
По названию сначала выводится 8 карточек; кнопка добавляет следующие 8 (8 → 16 → 24 …).
Цены подгружаются только для новых карточек и только по нажатию — API не нагружается зря.

---

## 🗂 Обслуживание библиотеки каталога (Parts-Catalogs)

Всё, что приходит от Parts-Catalogs, складывается в собственные таблицы
(`catalog_library_*`). Повторный заход на ту же машину обслуживается **из библиотеки,
без запросов к API**. Состояние: **96 машин / 87 деревьев узлов / 10 099 схем.**

### ⚠️ Сейчас сбор ОСТАНОВЛЕН

Тариф Parts-Catalogs израсходован (**999 из 1000**), после исчерпания сервис отключается
автоматически до 1-го числа. **Правила расхода запросов не подтверждены поставщиком:**
ранее в проекте было зафиксировано, что обход дерева узлов и сбор схем тариф не тратят
(на основании п. 1.2.2 договора и наблюдения «872 схемы на пакете в 100») — письмо
Tradesoft это опровергло. До письменного ответа, что именно считается «запросом»,
любой пакетный сбор остановлен:

```bash
pkill -f catalog_library_rebuild
```

Витрина при этом работает — она обслуживается из библиотеки.

### Полнота дерева узлов

В обходчике стояла зашитая константа `120` узлов — она резала **больше половины**
каталога. Замер настоящих размеров (`superadmin/pc_tree_probe.php`, только чтение):

| Машина | Реальных узлов | Было сохранено | Запросов `groups2` | Время |
|---|---|---|---|---|
| Lexus RX | **229** | 120 | 163 | ~81 сек |
| другие | до **745…1000** | 120 | — | — |

Лимит и глубина вынесены в настройки `site_settings`:

| Ключ | Что | Дефолт |
|---|---|---|
| `catalog_nodes_limit` | сколько узлов сохранять (потолок 2000) | `300` |
| `catalog_nodes_depth` | глубина обхода дерева | `5` |

Повышение лимита = больше HTTP-запросов и дольше обход — менять осознанно.

### CLI-инструменты (только на сервере)

```bash
php superadmin/catalog_library_rebuild.php plan      # что предстоит сделать
php superadmin/catalog_library_rebuild.php recount   # пересчёт деревьев под новый лимит
php superadmin/catalog_library_rebuild.php schemes   # догрузка схем
php superadmin/catalog_library_rebuild.php revin     # свежее декодирование VIN
php superadmin/catalog_library_rebuild.php all       # всё подряд (режим по умолчанию)

# необязательные аргументы: <режим> <машин за прогон=10> <схем за прогон=300> <пауза_мс=300>
php superadmin/catalog_library_rebuild.php recount 5 300 500

# замер настоящего размера дерева, без записи в БД
php superadmin/pc_tree_probe.php --lib                       # последнее авто из библиотеки
php superadmin/pc_tree_probe.php --vin <VIN>                 # по VIN (декодирование = запрос)
php superadmin/pc_tree_probe.php <catalog_id> <car_id> [criteria]

php superadmin/pc_quota_probe.php                    # заголовки ответа API (остаток квоты)
```

`catalog_library_rebuild.php` возобновляемый и защищён `flock` — **два параллельных
запуска удваивают расход тарифа** и гоняются за одни и те же машины.

### Протухающий токен комплектации

Parts-Catalogs зашивает в идентификатор авто временный токен комплектации
(`17*WA1VFAF82HA000123(2017!31a3cee7`). Он истекает, после чего `groups2`/`parts2`
отвечают `404 «Комплектация не найдена»` — и с параметром `criteria`, и без него.
Лечится **только новым декодированием VIN** (режим `revin`), а это стоит запроса тарифа.

### Конфиденциальность

Всё, что связано со сбором каталога, доступно **только superadmin/admin**. Покупателю на
`pages/vin.php` не показываются ни имя поставщика, ни лимиты — вместо технической ошибки
он видит «Каталог по этому автомобилю временно недоступен. Напишите менеджеру».

---

## 🏪 Маркетплейс (мультипродавец) — Фазы 1–3a

Площадка, где **сторонние продавцы выкладывают свои товары** (модель Ozon/WB) в нише
автозапчастей. **Завершены:** Фаза 1 (фундамент), Фаза 2 (заказы по продавцам)
и Фаза 3a (балансы и выплаты).

### Поток
```
Регистрация → выбор «Покупатель» или «Продавец»
   Продавец → users.role='seller' + sellers(status='pending')
      → Админ → Продавцы → «Одобрить»
         → Кабинет /seller/ → «Добавить товар»
            → parts(seller_id, moderation_status='pending')
               → Админ → Модерация товаров → «Одобрить»
                  → товар в каталоге; на карточке «Продавец: Магазин»
```

### Кабинет продавца `/seller/`
- **Обзор** — статус магазина с подсказкой, счётчики (всего / опубликовано / на проверке /
  отклонено), кнопка «Добавить товар» (заблокирована до одобрения), блок «Как это работает».
- **Мои товары** — статусы модерации + причина отклонения, изменить / удалить.
- **Добавить товар** — название, артикул, бренд, категория, цена, старая цена, наличие,
  вес, габариты, описание, до 6 фото. Любая правка снова уходит на проверку.

### Админка
| Раздел | Что можно |
|---|---|
| **Продавцы** (`admin/sellers.php`) | одобрить / заблокировать (с причиной), задать комиссию, фильтр по статусу |
| **Модерация товаров** (`admin/product_moderation.php`) | одобрить (публикация) / отклонить с причиной |

Оба пункта — в меню админа с бейджами количества «на проверке».

### Важно про видимость
Непроверенные листинги **не попадают** в каталог, поиск, на главную и не открываются по
прямой ссылке: везде добавлено условие
`(p.seller_id IS NULL OR p.moderation_status = 'active')`.
Товары **нашего** каталога (`seller_id IS NULL`) видны как раньше.

### Фаза 2 — заказы с разбивкой по продавцам

Один заказ покупателя может содержать товары нескольких продавцов и наши собственные.
При оформлении корзина **группируется по `parts.seller_id`** (NULL = наш каталог) и
для каждой группы создаётся **подзаказ** со своим статусом и своими деньгами.

```
orders (общий заказ покупателя: адрес, доставка, оплата)
  └── order_sellers (подзаказ: seller_id, status, subtotal,
  │                  commission_percent, commission_amount, payout_amount)
        └── order_items (строки товара: seller_id, order_seller_id)
```

**Миграция:**
```bash
mysql -u USER -p DBNAME < sql/marketplace_phase2.sql
```
Идемпотентна. Пока она не накачена, чекаут работает по-старому (плоская вставка) —
наличие таблицы определяется на лету.

| Кто | Что видит |
|---|---|
| **Продавец** (`/seller/orders.php`) | только свои подзаказы; смена статуса pending → processing → shipped → delivered, отмена. Адрес и телефон покупателя открываются **только после принятия заказа**. Все запросы жёстко со `WHERE seller_id = ?` |
| **Покупатель** (`/buyer/orders.php`) | заказ с разбивкой по продавцам и статусом каждой части |
| **Админ** (`/admin/orders.php`) | подзаказы + комиссия и сумма к выплате по каждому продавцу + итог платформы |

> **Комиссия фиксируется в момент заказа.** Процент копируется из `sellers` в
> `order_sellers.commission_percent`, поэтому последующая смена ставки не переписывает
> историю расчётов.

Счётчик новых заказов выведен на дашборд продавца.

⬜ Сквозная проверка Фазы 2 на боевом отложена заказчиком.

### Buy-box — этап 1: модель товара

Снимает главное ограничение маркетплейса: `parts.part_number` был **уникален по всей
базе**, поэтому если один продавец выложил артикул, второй выложить его уже не мог —
конкуренции продавцов не существовало.

```bash
php sql/migrate_buybox.php            # накат (повторный запуск безопасен)
...
После наката обязательно посмотрите `--report`: нормализация артикула склеивает
«90915-YZZE1» и «90915 YZZE1», и надо глазами убедиться, что в общей карточке
действительно один товар, а не две разные детали с похожими номерами. Если склеилось
ошибочно — пометьте лишний товар как «уникальный» либо исправьте у него артикул
и запустите `--regroup`.

**Модель.** Строка `parts` — это ПРЕДЛОЖЕНИЕ конкретного продавца (своя цена, наличие,
фото). Над ней два уровня:

```
семейство         «Автокресло Комфорт»            product_group_id
  └── карточка    «Комфорт, 1–3 года, чёрное»     product_id       ← buy-box тут
        └── предложения продавцов                 parts.id         ← это кладут в корзину
```

| Колонка | Что значит |
|---|---|
| `part_key` | артикул в каноническом виде (только буквы и цифры, верхний регистр) — по нему «90915-YZZE1» и «90915 yzze1» опознаются как один товар |
| `product_id` | номер общей карточки; **NULL = товар пока один в своём роде** |
| `product_group_id` | номер семейства (варианты: цвет, объём, возраст) — заполнится на этапе вариантов |
| `is_unique_item` | б/у, разборка, товар без артикула — никогда не сводится с чужими |

Ключ группы — `COALESCE(product_id, id)`, поэтому одиночный товар автоматически
«группа из самого себя» и нигде не нужен особый случай «а если NULL».

**Внешнего ключа на `product_id` намеренно нет:** это номер группы, а не ссылка на
строку. Если продавец-первопроходец удалит свой товар, остальные предложения обязаны
остаться сгруппированными — внешний ключ их бы раскидал.

Таблица `part_attributes` хранит гибкие атрибуты: `axis` — ось варианта (цвет, объём,
возрастная группа: своя цена и наличие, показывается переключателем), `spec` —
характеристика (вес брутто, страна, гарантия). Различать обязательно: «вес ребёнка» у
автокресла — ось, «вес посылки» — характеристика.

⚠️ Пересчёт (`--regroup`) при удалённом ведущем товаре сменит номер группы, а значит и
будущий адрес карточки. Поэтому это ручная операция, а не задача по расписанию.

### Buy-box — этап 2: витрина группирует предложения

Каталог, категории, главная, поиск и подсказки поиска показывают **одну карточку на
товар** и в ней — лучшее предложение. Дубли одного товара от разных продавцов больше
не занимают несколько плиток подряд.

**Правило выбора победителя:** сначала «в наличии», среди них — дешевле. Намеренно
совпадает с правилом для предложений AutoEuro (`offersByOem`), чтобы покупатель не
видел на одной витрине две разные логики.

**Одно предложение от одного продавца на карточку.** Продавец не конкурирует сам с
собой: два его предложения на один артикул — это дубль в данных, а не выбор для
покупателя (так же устроены Ozon и Amazon). Такие дубли помечаются в Админ → Товары
красной подписью «дубль».
...
который есть у второго продавца.

Счётчики и пагинация считают **карточки**, а не предложения — иначе «найдено 120», а на
страницах их 80.

Если сервер не умеет оконные функции (нужен MySQL 8+ / MariaDB 10.2+), витрина
автоматически работает как раньше — плоским списком, без группировки. Проверяется
пробным запросом при первом обращении.

**Попутно исправлено:**
- `catalog/category.php` и `api/search.php` **не проверяли модерацию** — неодобренные
  листинги продавцов были видны на странице категории и в подсказках поиска;
- поиск фото для карточек поставщика переведён на индексированный `part_key`: раньше
  нормализация считалась прямо в запросе (`REPLACE(REPLACE(...))`), из-за чего индекс не
  использовался — полный перебор таблицы на каждую карточку, — а правило расходилось с
  `partsCanonicalArticle()`.

### Buy-box — этап 3: предложения продавцов на карточке товара

Когда товар продают несколько магазинов, на странице появляется блок **«Этот товар
продают N продавцов»**: у каждого видна цена, наличие и кнопка «выбрать». Победитель
buy-box помечен «лучшее предложение» — это ровно то, что показано в каталоге.

Строки блока — **обычные ссылки** на страницу соответствующего предложения, а не
радиокнопки на JS: работает без скриптов, ссылки индексируются и не ломаются.
Открытое сейчас предложение подсвечено и не кликается.

Дубли одного продавца в список не попадают — как и на витрине, один магазин
...
`canonical` поисковик считал бы их дублями и размазывал вес между ними.

**Попутно исправлено:** блок «похожие товары» не проверял модерацию (третий такой
пропуск) и не группировал предложения — один товар от трёх продавцов мог занять всю
подборку. Заодно из «похожих» теперь исключается вся карточка целиком, а не только
открытое предложение: раньше «похожим» мог оказаться тот же самый товар от соседа.

### Buy-box — этап 4: формы товаров

Снят запрет, из-за которого маркетплейс не мог работать. Раньше проверка была
«артикул уникален по всей базе»: выложил товар один продавец — второй получал отказ.

**Новое правило:** один и тот же артикул у **разных** продавцов — норма, они и должны
конкурировать. Ошибка — когда **один** продавец выкладывает его себе дважды: тогда у
одной детали оказываются две цены от одного магазина. Наш собственный каталог считается
таким же «продавцом», поэтому правило действует и в админке.

При сохранении товар автоматически получает `part_key` и, если он не уникальный,
подставляется в общую карточку — продавцу ничего указывать не нужно.

**Галочка «Уникальный товар»** — б/у, разборка, позиция без заводского артикула. Такой
товар получает отдельную карточку и никогда не объединяется с чужими: двигатель с
разборки от двух продавцов — это два разных товара, а не два предложения на один.
Галочка есть во всех трёх формах (продавец, админ, менеджер), поэтому правка товара не
сбрасывает флаг.

### Buy-box — этап 7: тестовые продавцы

Механика конкуренции продавцов на боевом невидима: продавец там один — наш каталог, а
блок «этот товар продают N продавцов» появляется только при двух и более предложениях.
Скрипт заводит магазины и выкладывает им **те же артикулы**, что уже есть у нас.
...
Вход в кабинет: `seller1@test.local` … пароль `Test12345!`

**Что получается на карточке** (цены пляшут вокруг нашей):

| Продавец | Цена | Наличие | |
|---|---|---|---|
| Магазин 1 | 539.40 | в наличии | ← победитель buy-box |
| Магазин 2 | 613.80 | в наличии | |
| Наш каталог | 620.00 | в наличии | |
| Магазин 3 | **341.00** | под заказ | самый дешёвый, но последний |

Магазину без наличия намеренно дана **самая низкая цена**: так проверка доказывает
правило — побеждает не самый дешёвый вообще, а самый дешёвый **из тех, у кого товар
есть**. При неверной сортировке проверка бы это не поймала.

Комиссия у каждого магазина своя (7%, 9%, 11%) — видно, что она фиксируется отдельно
по каждому продавцу в заказах и выплатах.

**Безопасность.** Скрипт трогает только то, что создал сам: аккаунты опознаются по
e-mail `…@test.local`, товары удаляются только по `seller_id` этих магазинов. Наш
каталог (`seller_id IS NULL`) и настоящие продавцы не затрагиваются — проверено тестом.
Удаление требует явного `--yes`.

### Buy-box — этап 5: варианты товара (цвет, размер, возраст)

Продавец связывает свои исполнения одного товара — чехлы чёрные и бежевые, автокресло
0–1 и 1–3 года, масло 1 л и 4 л. На карточке появляется переключатель: у каждого
варианта своя цена, наличие и фото.

**Семейство задаётся ссылкой на свой же товар**, а не текстовым названием модели. Тогда
не нужна ни отдельная колонка-ключ, ни борьба с опечатками («Комфорт» против «комфорт »):
продавец просто выбирает товар из списка своих. Поле — «Вариант товара» в форме.

**Атрибуты гибкие** — пара «название → значение», а не жёсткие колонки. Два вида:

| Вид | Примеры | Что делает |
|---|---|---|
| **Ось варианта** | цвет, объём, возрастная группа | отличает исполнения; **своя цена и наличие**, рисуется переключателем |
| **Характеристика** | вес брутто, страна, гарантия | просто описание в таблице |

Различать обязательно: «вес ребёнка» у автокресла — ось, «вес посылки» — характеристика.

Семейство принадлежит **одному продавцу**: варианты — это его линейка исполнений, а за
конкуренцию отвечает `product_id`. Чужой товар как родителя система не принимает.

⚠️ Ключ семейства — `COALESCE(product_group_id, id)` и намеренно **не** падает на
`product_id`. Карточка и семейство — разные оси: у главного варианта `product_group_id`
пуст, а `product_id` вполне может быть заполнен (тот же артикул продаёт кто-то ещё).
Тогда его ключ семейства стал бы номером чужой карточки, и семейство развалилось бы.

### Buy-box — этап 6: каталог группирует по семейству

Чехлы в пяти цветах больше не занимают пять плиток: при **просмотре** каталога линейка
схлопывается в одну, а рядом с брендом появляется пометка «5 вариантов».

**В поиске группировка намеренно выключена.** Человек ищет «бежевый чехол» — если
схлопнуть семейство, победителем окажется чёрный, и найденное подменится другим товаром.
Поэтому `partsBuyBoxSource(..., $groupFamily)`: `true` в каталоге, на категории и на
главной; `false` в поиске и подсказках.

Отбор идёт снизу вверх: предложение → карточка (buy-box между продавцами) → семейство
(варианты одного продавца).

⚠️ **Известное ограничение.** Семейство принадлежит продавцу, а победитель карточки
может оказаться из другого магазина. Тогда линейка разрывается: если чёрные чехлы
магазина А проиграли по цене магазину Б, в каталоге будут две плитки — «бежевые (А)» и
«чёрные (Б)» — вместо одной. Ничего не теряется: чёрные от А доступны в блоке
предложений на карточке. Но число плиток зависит от того, кто выиграл buy-box.

Это следствие того, что варианты живут у продавца, а не на общей карточке. Устраняется
только переносом вариантов на карточку — полная модель Ozon, отдельная задача. Пока
показывать вариант конкурента внутри чужой линейки было бы хуже, чем разорванная линейка.

### Фаза 4 — отзывы о продавцах

Ни один из существовавших видов отзывов не отвечал на вопрос **«каков этот продавец»**:
`product_reviews` — о товаре, `shop_reviews` — о нашем магазине в целом. Без такой оценки
покупателю не на что опереться при выборе между предложениями на одной карточке.

```bash
php sql/apply.php sql/marketplace_phase4_reviews.sql
```

**Отзыв привязан к выполненному подзаказу**, а не просто к продавцу. Это одновременно
доказательство покупки и защита от накрутки: оценить можно только то, что реально
доставили, и ровно один раз на заказ. Купил трижды — оставишь три отзыва.

| Где | Что видно |
|---|---|
| Карточка товара | звёзды рядом с каждым продавцом в блоке предложений |
| Страница магазина | рейтинг в шапке + список отзывов с ответами продавца |
| Кабинет покупателя | форма оценки под доставленным подзаказом |
| Менеджер → Отзывы | третья вкладка «Отзывы о продавцах» со счётчиком |

В рейтинг идут **только одобренные** отзывы: непроверенный отзыв не должен влиять на то,
чьё предложение покупатель увидит первым.

**Попутно исправлено:** рейтинг и список отзывов о товаре считались **по предложению**.
После buy-box один товар продают несколько магазинов, и отзывы разъезжались по их
строкам — на карточке победителя оказалось бы два отзыва вместо двадцати. Теперь
агрегация идёт по карточке. Это тот дефект, который был отмечен ещё на этапе 3 buy-box.

### Фаза 4 — возвраты и споры

До этого после доставки заказ считался закрытым навсегда. Если товар не подошёл или
пришёл сломанным, покупателю оставалось писать в чат, а деньги продавцу уже начислены —
снять их можно было только ручной корректировкой, без следа о причине.

```bash
php sql/migrate_returns.php
```

**Поток:** покупатель оформляет возврат под доставленным подзаказом (целиком или по одной
позиции) → продавец в разделе «Возвраты» одобряет или отказывает → если покупатель не
согласен с отказом, администратор может **пересмотреть** заявку.

Отдельной сущности «спор» намеренно нет: это то же обращение, рассмотренное второй
инстанцией. Заводить ради этого параллельную таблицу значило бы раздвоить историю одного
и того же события.

**Про деньги — момент, который легко перепутать.** Покупателю возвращают полную стоимость
товара, а с баланса продавца снимают **только его долю** — то, что ему начислили за
вычетом комиссии:

| Подзаказ | Выплата продавцу | Возврат позиции | Снимется с продавца |
|---|---|---|---|
| 1000 | 900 (комиссия 10%) | 250 | **225** |
| 1000 | 900 | 1000 (всё) | **900** |

Снять полную стоимость значило бы заставить продавца оплатить ещё и комиссию площадки за
несостоявшуюся продажу. Продавец видит эту сумму **до** нажатия кнопки — решение о
деньгах не должно быть сюрпризом.

Движение пишется в журнал типом `refund` со **ссылкой на заявку**: без неё в балансе
висело бы «минус 225» без объяснения, и через полгода такую строку невозможно оспорить.
Одобрение и списание идут одной транзакцией — «возврат одобрен, но деньги не сняты»
худшее из состояний, потому что заметят его нескоро.

### Фаза 3b — каркас приёма оплаты («деньги внутрь»)

Эквайринга в Таджикистане у нас пока нет: нужен договор с банком и его песочница. Но всё,
что **не зависит от конкретного банка**, построено заранее — когда договор появится,
останется написать один адаптер по документации, а не поднимать платёжный контур с нуля.

```bash
php sql/migrate_payments.php
```

**Что даёт уже сейчас, без банка.** `orders.payment_status` отвечает на вопрос, который
до сих пор нигде не хранился: получены ли деньги. Наличные тоже бывают получены и нет, а
заказ до этого был просто «доставлен». Владелец отмечает оплату в **Админ → Оплаты**, и
она попадает в тот же журнал, куда потом будет писать банк.

Статус оплаты отделён от статуса заказа намеренно: заказ может быть доставлен, но не
оплачен (спор), и наоборот — оплачен онлайн, но ещё не собран.

**Три вещи, которые нельзя доверять адаптеру банка**, поэтому они в общем слое:

| Что | Почему здесь |
|---|---|
| **Сверка суммы** | если бы каждый адаптер проверял сам, однажды какой-нибудь забыл бы — и заказ на 5000 закрылся бы платежом на 5 |
| **Идемпотентность** | банки шлют подтверждение по нескольку раз, это норма; повторное не должно ни задваивать платёж, ни ронять обработчик |
| **Журнал попыток** | «покупатель говорит, что платил» без журнала невозможно ни подтвердить, ни опровергнуть; сырой ответ банка хранится как есть — в претензии он оперирует своим форматом, а не нашим |

Подпись проверяет адаптер (`handleCallback`) — схема у каждого банка своя.

**Чего каркас не делает и не может:** маппинг запросов конкретного банка, формат подписи,
коды статусов. Это описано в его документации, и угадывать её значит написать
абстракцию, которую придётся переделать.

**Как добавить банк, когда будет договор:** новый класс с интерфейсом `PaymentProvider`,
одна строка в `paymentProviders()`, настройка `payment_provider`. Чекаут и админка не
меняются.

### Фаза 3 — балансы и выплаты продавцам («деньги наружу»)

Фаза 2 научилась считать комиссию по каждому подзаказу, но эти цифры нигде не
накапливались: продавец не видел, сколько ему должны, а у владельца не было реестра
выплат. Эта часть закрывает расчёты — **эквайринг для неё не нужен**, деньги площадка
...
```
seller_ledger    каждая строка — движение денег со знаком:
                 earning +  начислено за доставленный подзаказ (сумма − комиссия)
                 reversal −  сторно, если доставленный заказ отменили
                 payout   −  выплата продавцу
                 adjustment ± ручная правка владельцем (штраф, компенсация, спор)

баланс продавца = SUM(amount)          ← всегда пересчитывается, испортить нельзя
seller_payouts   факт выплаты: сумма, способ, номер перевода, период, кто провёл
```

**Правила начисления:**
- деньги начисляются, когда подзаказ переходит в **«Доставлен»** — не раньше;
- сумма = `payout_amount`, зафиксированная в момент заказа;
- отмена уже доставленного подзаказа сторнируется в минус;
- наш собственный каталог (`seller_id IS NULL`) в расчётах не участвует — площадка
  не должна денег сама себе;
- повторный клик по «Доставлен» долг **не удваивает** (UNIQUE в журнале).

| Кто | Где | Что делает |
|---|---|---|
| **Продавец** | `/seller/finance.php` | видит «К выплате сейчас», сколько в пути, удержанную комиссию, историю начислений и все выплаты. Только чтение |
| **Владелец** | `/admin/payouts.php` | реестр «кому сколько должны», кнопка «Выплатить» (способ, номер перевода, период), ручная корректировка с обязательной причиной, выгрузка CSV для бухгалтерии |

Кнопка **«Пересчитать начисления»** в реестре — для заказов, доставленных до появления
журнала, и на случай правок статуса напрямую в БД. Полностью идемпотентна.

Переплату система не пропускает: выплатить больше остатка нельзя, иначе баланс уходит
в минус и потом не отличить долг продавца от опечатки.

### Что осталось до полноценного маркетплейса
| Фаза | Что | Статус |
|---|---|---|
| 1 | Продавцы, их товары, модерация, витрина магазина | ✅ готово |
| 2 | Заказы: разбивка по продавцам, кабинет заказов продавца, статусы, комиссия | ✅ готово (проверка на боевом отложена) |
| 3a | **Балансы и выплаты продавцам** («деньги наружу») | ✅ готово |
| 2b | **buy-box** — одна деталь, много продавцов, выбор лучшего предложения | ✅ готово (этапы 1–7) |
| 3b | Онлайн-оплата картой/QR («деньги внутрь») | ⬜ (нужен эквайринг в ТJ) |
| 4 | Отзывы о продавцах, возвраты/споры, KYC, аналитика | ⬜ |

> Ограничение снято: `parts.part_number` больше не уникален по всей базе. Один артикул у
> разных продавцов — норма, у одного продавца дважды — нельзя. См. раздел «Buy-box».
>
> ⚠️ Что осталось от buy-box: варианты принадлежат продавцу, а не общей карточке, поэтому
> если карточку выиграл конкурент, линейка продавца разрывается на несколько плиток.
> Лечится переносом вариантов на карточку — полная модель Ozon, отдельная задача.
