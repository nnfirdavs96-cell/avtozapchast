...
дней, поэтому повторные запросы того же VIN мгновенны и не тратят лимит.

Ответ группы: `[{group, name, parts:"БРЕНД|АРТИКУЛ", shortname}]` — **без цен**.
Если артикул совпал с товаром на своём складе (`parts.part_number`) —
подставляется своя цена, наличие и кнопка «в корзину»; иначе позиция
показывается как «под заказ».
...
> Миграций БД не требуется: настройки — в `site_settings`, кэш-таблица
> `partsapi_catalog_cache` создаётся автоматически при первом обращении.
...
- `includes/catalog_api.php` — класс `CatalogApi`: перебор групп
  `getPartsbyVIN`, разбор `БРЕНД|АРТИКУЛ`, обогащение со склада, серверный
  кэш. Инертен, пока `catalog_api_enabled=0`.
- `includes/vin_service.php` — провайдер `partsapi` понимает реальный формат
  `VINdecodeOE` (`{"data":{"array":{…}}}` с транслит-ключами `brend`,
  `naimenovanie`, `modely`, `modifikaciya`, `data`, `rynok`).
- `api/vin_catalog.php` — AJAX-эндпоинт: отдаёт собранный каталог по VIN.
- `pages/vin.php` — карточка авто рендерится сразу, каталог подгружается
  **асинхронно** (перебор групп долгий), группируется по узлам.
...
3. Жмёт **«Проверить соединение»** — видит, что запчасти приходят.
