...
function renderRoleSidebar(string $active = ''): void {
    ...
    // Заявки на возврат, ждущие решения. Таблица появляется только после миграции
    // Фазы 4, поэтому запрос отдельный и в своём try.
    $returnsPending = 0;
    try {
        $returnsPending = (int)getDB()->query(
            "SELECT COUNT(*) FROM order_returns WHERE status='requested'"
        )->fetchColumn();
...
/**
 * Пометка «этот товар есть ещё у N продавцов».
 *
 * Рисуется, только когда предложений больше одного, поэтому в каталоге без
 * продавцов не появляется вовсе. Считаются ПРОДАВЦЫ, а не строки: дубли одного
 * продавца витрина прячет и в счёт не берёт (см. partsBuyBoxSource).
 *
 * `offers_count` приходит из источника витрины; на страницах, которые его не
